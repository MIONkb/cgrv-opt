...
func.func @forward(%arg0: memref<4x3xf32>) -> memref<4x3xf32> {
  ...
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1, d0)>], iterator_types = ["parallel", "parallel"]} ins(%0 : memref<3x3xf32>) outs(%alloca : memref<3x3xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  cf.assert %true, "mismatching contracting dimension for torch.aten.mm"
  ...
  %c0 = arith.constant 0 : index
  %dim = memref.dim %alloc, %c0 : memref<?x?xf32>
  %c1 = arith.constant 1 : index
  %dim_0 = memref.dim %alloc, %c1 : memref<?x?xf32>
  %alloc_1 = memref.alloc(%dim, %dim_0) {alignment = 64 : i64} : memref<?x?xf32>
  memref.copy %alloc, %alloc_1 : memref<?x?xf32> to memref<?x?xf32>
  linalg.matmul ins(%arg0, %alloca : memref<4x3xf32>, memref<3x3xf32>) outs(%alloc_1 : memref<?x?xf32>)
