#map = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %c0 = arith.constant 0 : index
    %c0_0 = arith.constant 0 : index
    %cst = arith.constant 0.000000e+00 : f32
    %cst_1 = arith.constant 3.40282347E+38 : f32
    %cst_2 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %1 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %2 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %3 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %4 = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    %5 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    %c0_3 = arith.constant 0 : index
    %c4 = arith.constant 4 : index
    %c1 = arith.constant 1 : index
    scf.for %arg2 = %c0_3 to %c4 step %c1 {
      %c0_57 = arith.constant 0 : index
      %c35 = arith.constant 35 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c35 step %c1_58 {
        %c0_59 = arith.constant 0 : index
        %c35_60 = arith.constant 35 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_59 to %c35_60 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c1_63 = arith.constant 1 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c1_63 step %c1_64 {
            memref.store %cst, %5[%arg2, %arg3, %arg4, %arg5] : memref<4x35x35x1xf32>
          }
        }
      }
    }
    %6 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %5, %6 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %5 : memref<4x35x35x1xf32>
    %7 = memref.subview %6[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map>
    memref.copy %arg0, %7 : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map>
    %8 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    %c0_4 = arith.constant 0 : index
    %c5 = arith.constant 5 : index
    %c1_5 = arith.constant 1 : index
    scf.for %arg2 = %c0_4 to %c5 step %c1_5 {
      %c0_57 = arith.constant 0 : index
      %c5_58 = arith.constant 5 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c5_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c1_61 = arith.constant 1 : index
        %c1_62 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c1_61 step %c1_62 {
          %c0_63 = arith.constant 0 : index
          %c1_64 = arith.constant 1 : index
          %c1_65 = arith.constant 1 : index
          scf.for %arg5 = %c0_63 to %c1_64 step %c1_65 {
            %38 = memref.load %3[%arg2, %arg3, %c0, %c0_0] : memref<5x5x1x1xf32>
            memref.store %38, %4[%c0_0, %arg2, %arg3, %c0] : memref<1x5x5x1xf32>
            %39 = memref.load %4[%arg5, %arg2, %arg3, %arg4] : memref<1x5x5x1xf32>
            memref.store %39, %8[%arg2, %arg3, %arg4, %arg5] : memref<5x5x1x1xf32>
          }
        }
      }
    }
    memref.dealloc %4 : memref<1x5x5x1xf32>
    %9 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    %c0_6 = arith.constant 0 : index
    %c4_7 = arith.constant 4 : index
    %c1_8 = arith.constant 1 : index
    scf.for %arg2 = %c0_6 to %c4_7 step %c1_8 {
      %c0_57 = arith.constant 0 : index
      %c16 = arith.constant 16 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c16 step %c1_58 {
        %c0_59 = arith.constant 0 : index
        %c16_60 = arith.constant 16 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_59 to %c16_60 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c1_63 = arith.constant 1 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c1_63 step %c1_64 {
            memref.store %cst, %9[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    %10 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %9, %10 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %9 : memref<4x16x16x1xf32>
    %c0_9 = arith.constant 0 : index
    %c4_10 = arith.constant 4 : index
    %c1_11 = arith.constant 1 : index
    scf.for %arg2 = %c0_9 to %c4_10 step %c1_11 {
      %c0_57 = arith.constant 0 : index
      %c16 = arith.constant 16 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c16 step %c1_58 {
        %c0_59 = arith.constant 0 : index
        %c16_60 = arith.constant 16 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_59 to %c16_60 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c1_63 = arith.constant 1 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c1_63 step %c1_64 {
            %c0_65 = arith.constant 0 : index
            %c1_66 = arith.constant 1 : index
            %c1_67 = arith.constant 1 : index
            scf.for %arg6 = %c0_65 to %c1_66 step %c1_67 {
              %c0_68 = arith.constant 0 : index
              %c5_69 = arith.constant 5 : index
              %c1_70 = arith.constant 1 : index
              scf.for %arg7 = %c0_68 to %c5_69 step %c1_70 {
                %c0_71 = arith.constant 0 : index
                %c5_72 = arith.constant 5 : index
                %c1_73 = arith.constant 1 : index
                scf.for %arg8 = %c0_71 to %c5_72 step %c1_73 {
                  %c2 = arith.constant 2 : index
                  %38 = arith.muli %arg3, %c2 : index
                  %39 = arith.addi %38, %arg7 : index
                  %c2_74 = arith.constant 2 : index
                  %40 = arith.muli %arg4, %c2_74 : index
                  %41 = arith.addi %40, %arg8 : index
                  %42 = memref.load %6[%arg2, %39, %41, %arg6] : memref<4x35x35x1xf32>
                  %43 = memref.load %8[%arg7, %arg8, %arg6, %arg5] : memref<5x5x1x1xf32>
                  %44 = memref.load %10[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
                  %45 = arith.mulf %42, %43 : f32
                  %46 = arith.addf %44, %45 : f32
                  memref.store %46, %10[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
                }
              }
            }
          }
        }
      }
    }
    memref.dealloc %8 : memref<5x5x1x1xf32>
    memref.dealloc %6 : memref<4x35x35x1xf32>
    %11 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    %c0_12 = arith.constant 0 : index
    %c4_13 = arith.constant 4 : index
    %c1_14 = arith.constant 1 : index
    scf.for %arg2 = %c0_12 to %c4_13 step %c1_14 {
      %c0_57 = arith.constant 0 : index
      %c16 = arith.constant 16 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c16 step %c1_58 {
        %c0_59 = arith.constant 0 : index
        %c16_60 = arith.constant 16 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_59 to %c16_60 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c1_63 = arith.constant 1 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c1_63 step %c1_64 {
            %38 = memref.load %10[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
            %39 = arith.addf %38, %cst : f32
            memref.store %39, %11[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    memref.dealloc %10 : memref<4x16x16x1xf32>
    %12 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    %c0_15 = arith.constant 0 : index
    %c4_16 = arith.constant 4 : index
    %c1_17 = arith.constant 1 : index
    scf.for %arg2 = %c0_15 to %c4_16 step %c1_17 {
      %c0_57 = arith.constant 0 : index
      %c16 = arith.constant 16 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c16 step %c1_58 {
        %c0_59 = arith.constant 0 : index
        %c16_60 = arith.constant 16 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_59 to %c16_60 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c1_63 = arith.constant 1 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c1_63 step %c1_64 {
            %38 = memref.load %11[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
            %39 = arith.cmpf olt, %38, %cst : f32
            %40 = arith.select %39, %cst, %38 : f32
            %41 = arith.cmpf olt, %cst_1, %38 : f32
            %42 = arith.select %41, %cst_1, %40 : f32
            memref.store %42, %12[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    memref.dealloc %11 : memref<4x16x16x1xf32>
    %13 = memref.collapse_shape %12 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %14 = memref.expand_shape %13 [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    %c0_18 = arith.constant 0 : index
    %c1_19 = arith.constant 1 : index
    %c1_20 = arith.constant 1 : index
    scf.for %arg2 = %c0_18 to %c1_19 step %c1_20 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c8 = arith.constant 8 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c8 step %c1_61 {
          memref.store %cst, %15[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
        }
      }
    }
    %16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %15, %16 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %15 : memref<1x4x8xf32>
    %c0_21 = arith.constant 0 : index
    %c1_22 = arith.constant 1 : index
    %c1_23 = arith.constant 1 : index
    scf.for %arg2 = %c0_21 to %c1_22 step %c1_23 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c8 = arith.constant 8 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c8 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c256 = arith.constant 256 : index
          %c1_63 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c256 step %c1_63 {
            %38 = memref.load %14[%arg2, %arg3, %arg5] : memref<1x4x256xf32>
            %39 = memref.load %2[%arg2, %arg5, %arg4] : memref<1x256x8xf32>
            %40 = memref.load %16[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
            %41 = arith.mulf %38, %39 : f32
            %42 = arith.addf %40, %41 : f32
            memref.store %42, %16[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
          }
        }
      }
    }
    memref.dealloc %12 : memref<4x16x16x1xf32>
    %17 = memref.collapse_shape %16 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %18 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    memref.dealloc %16 : memref<1x4x8xf32>
    %19 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    %c0_24 = arith.constant 0 : index
    %c4_25 = arith.constant 4 : index
    %c1_26 = arith.constant 1 : index
    scf.for %arg2 = %c0_24 to %c4_25 step %c1_26 {
      %c0_57 = arith.constant 0 : index
      %c8 = arith.constant 8 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c8 step %c1_58 {
        %38 = memref.load %17[%arg2, %arg3] : memref<4x8xf32>
        %39 = arith.addf %38, %cst : f32
        memref.store %39, %18[%arg2, %arg3] : memref<4x8xf32>
        %40 = memref.load %18[%arg2, %arg3] : memref<4x8xf32>
        %41 = arith.cmpf olt, %40, %cst : f32
        %42 = arith.select %41, %cst, %40 : f32
        %43 = arith.cmpf olt, %cst_1, %40 : f32
        %44 = arith.select %43, %cst_1, %42 : f32
        memref.store %44, %19[%arg2, %arg3] : memref<4x8xf32>
      }
    }
    memref.dealloc %18 : memref<4x8xf32>
    %20 = memref.expand_shape %19 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    %c0_27 = arith.constant 0 : index
    %c1_28 = arith.constant 1 : index
    %c1_29 = arith.constant 1 : index
    scf.for %arg2 = %c0_27 to %c1_28 step %c1_29 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c4_61 = arith.constant 4 : index
        %c1_62 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c4_61 step %c1_62 {
          memref.store %cst, %21[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
        }
      }
    }
    %22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %21, %22 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %21 : memref<1x4x4xf32>
    %c0_30 = arith.constant 0 : index
    %c1_31 = arith.constant 1 : index
    %c1_32 = arith.constant 1 : index
    scf.for %arg2 = %c0_30 to %c1_31 step %c1_32 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c4_61 = arith.constant 4 : index
        %c1_62 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c4_61 step %c1_62 {
          %c0_63 = arith.constant 0 : index
          %c8 = arith.constant 8 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_63 to %c8 step %c1_64 {
            %38 = memref.load %20[%arg2, %arg3, %arg5] : memref<1x4x8xf32>
            %39 = memref.load %1[%arg2, %arg5, %arg4] : memref<1x8x4xf32>
            %40 = memref.load %22[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
            %41 = arith.mulf %38, %39 : f32
            %42 = arith.addf %40, %41 : f32
            memref.store %42, %22[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
          }
        }
      }
    }
    memref.dealloc %19 : memref<4x8xf32>
    %23 = memref.collapse_shape %22 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %24 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    memref.dealloc %22 : memref<1x4x4xf32>
    %25 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    %c0_33 = arith.constant 0 : index
    %c4_34 = arith.constant 4 : index
    %c1_35 = arith.constant 1 : index
    scf.for %arg2 = %c0_33 to %c4_34 step %c1_35 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %38 = memref.load %23[%arg2, %arg3] : memref<4x4xf32>
        %39 = arith.addf %38, %cst : f32
        memref.store %39, %24[%arg2, %arg3] : memref<4x4xf32>
        %40 = memref.load %24[%arg2, %arg3] : memref<4x4xf32>
        %41 = arith.cmpf olt, %40, %cst : f32
        %42 = arith.select %41, %cst, %40 : f32
        %43 = arith.cmpf olt, %cst_1, %40 : f32
        %44 = arith.select %43, %cst_1, %42 : f32
        memref.store %44, %25[%arg2, %arg3] : memref<4x4xf32>
      }
    }
    memref.dealloc %24 : memref<4x4xf32>
    %26 = memref.expand_shape %25 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %27 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    %c0_36 = arith.constant 0 : index
    %c1_37 = arith.constant 1 : index
    %c1_38 = arith.constant 1 : index
    scf.for %arg2 = %c0_36 to %c1_37 step %c1_38 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c2 = arith.constant 2 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c2 step %c1_61 {
          memref.store %cst, %27[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
        }
      }
    }
    %28 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %27, %28 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %27 : memref<1x4x2xf32>
    %c0_39 = arith.constant 0 : index
    %c1_40 = arith.constant 1 : index
    %c1_41 = arith.constant 1 : index
    scf.for %arg2 = %c0_39 to %c1_40 step %c1_41 {
      %c0_57 = arith.constant 0 : index
      %c4_58 = arith.constant 4 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c4_58 step %c1_59 {
        %c0_60 = arith.constant 0 : index
        %c2 = arith.constant 2 : index
        %c1_61 = arith.constant 1 : index
        scf.for %arg4 = %c0_60 to %c2 step %c1_61 {
          %c0_62 = arith.constant 0 : index
          %c4_63 = arith.constant 4 : index
          %c1_64 = arith.constant 1 : index
          scf.for %arg5 = %c0_62 to %c4_63 step %c1_64 {
            %38 = memref.load %26[%arg2, %arg3, %arg5] : memref<1x4x4xf32>
            %39 = memref.load %0[%arg2, %arg5, %arg4] : memref<1x4x2xf32>
            %40 = memref.load %28[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
            %41 = arith.mulf %38, %39 : f32
            %42 = arith.addf %40, %41 : f32
            memref.store %42, %28[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
          }
        }
      }
    }
    memref.dealloc %25 : memref<4x4xf32>
    %29 = memref.collapse_shape %28 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %30 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    memref.dealloc %28 : memref<1x4x2xf32>
    %31 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    %c0_42 = arith.constant 0 : index
    %c4_43 = arith.constant 4 : index
    %c1_44 = arith.constant 1 : index
    scf.for %arg2 = %c0_42 to %c4_43 step %c1_44 {
      %c0_57 = arith.constant 0 : index
      %c2 = arith.constant 2 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c2 step %c1_58 {
        %38 = memref.load %29[%arg2, %arg3] : memref<4x2xf32>
        %39 = arith.addf %38, %cst : f32
        memref.store %39, %30[%arg2, %arg3] : memref<4x2xf32>
        %40 = memref.load %30[%arg2, %arg3] : memref<4x2xf32>
        %41 = math.exp %40 : f32
        memref.store %41, %31[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    memref.dealloc %30 : memref<4x2xf32>
    %32 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    %c0_45 = arith.constant 0 : index
    %c4_46 = arith.constant 4 : index
    %c1_47 = arith.constant 1 : index
    scf.for %arg2 = %c0_45 to %c4_46 step %c1_47 {
      memref.store %cst, %32[%arg2] : memref<4xf32>
    }
    %33 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %32, %33 : memref<4xf32> to memref<4xf32>
    memref.dealloc %32 : memref<4xf32>
    %c0_48 = arith.constant 0 : index
    %c4_49 = arith.constant 4 : index
    %c1_50 = arith.constant 1 : index
    scf.for %arg2 = %c0_48 to %c4_49 step %c1_50 {
      %c0_57 = arith.constant 0 : index
      %c2 = arith.constant 2 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c2 step %c1_58 {
        %38 = memref.load %31[%arg2, %arg3] : memref<4x2xf32>
        %39 = memref.load %33[%arg2] : memref<4xf32>
        %40 = arith.addf %38, %39 : f32
        memref.store %40, %33[%arg2] : memref<4xf32>
      }
    }
    %34 = memref.expand_shape %33 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %35 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    %c0_51 = arith.constant 0 : index
    %c4_52 = arith.constant 4 : index
    %c1_53 = arith.constant 1 : index
    scf.for %arg2 = %c0_51 to %c4_52 step %c1_53 {
      %c0_57 = arith.constant 0 : index
      %c1_58 = arith.constant 1 : index
      %c1_59 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c1_58 step %c1_59 {
        %38 = memref.load %34[%arg2, %arg3] : memref<4x1xf32>
        %39 = arith.divf %cst_2, %38 : f32
        memref.store %39, %35[%arg2, %arg3] : memref<4x1xf32>
      }
    }
    memref.dealloc %33 : memref<4xf32>
    %36 = memref.collapse_shape %35 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %37 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    %c0_54 = arith.constant 0 : index
    %c4_55 = arith.constant 4 : index
    %c1_56 = arith.constant 1 : index
    scf.for %arg2 = %c0_54 to %c4_55 step %c1_56 {
      %c0_57 = arith.constant 0 : index
      %c2 = arith.constant 2 : index
      %c1_58 = arith.constant 1 : index
      scf.for %arg3 = %c0_57 to %c2 step %c1_58 {
        %38 = memref.load %31[%arg2, %arg3] : memref<4x2xf32>
        %39 = memref.load %36[%arg2] : memref<4xf32>
        %40 = arith.mulf %38, %39 : f32
        memref.store %40, %37[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    memref.dealloc %35 : memref<4x1xf32>
    memref.dealloc %31 : memref<4x2xf32>
    memref.copy %37, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
}

