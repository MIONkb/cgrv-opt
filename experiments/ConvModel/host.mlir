#map0 = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
#map1 = affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>
#map2 = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
#map3 = affine_map<(d0, d1, d2, d3) -> (d3)>
#map4 = affine_map<(d0, d1) -> (d0, d1)>
#map5 = affine_map<(d0, d1) -> (d1)>
#map6 = affine_map<(d0, d1) -> (d0)>
module attributes {soda.container_module, tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  func.func private @main_kernel_main_kernel(memref<5x5x1x1xf32>, memref<1x5x5x1xf32>)
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    %cst_1 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
    %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
    %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
    %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
    %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %8 = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    call @main_kernel_main_kernel(%7, %8) : (memref<5x5x1x1xf32>, memref<1x5x5x1xf32>) -> ()
    %9 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    linalg.fill ins(%cst : f32) outs(%9 : memref<4x35x35x1xf32>)
    %10 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %9, %10 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %9 : memref<4x35x35x1xf32>
    %11 = memref.subview %10[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map0>
    memref.copy %arg0, %11 : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map0>
    %12 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    linalg.generic {indexing_maps = [#map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%8 : memref<1x5x5x1xf32>) outs(%12 : memref<5x5x1x1xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      linalg.yield %arg2 : f32
    }
    memref.dealloc %8 : memref<1x5x5x1xf32>
    %13 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.fill ins(%cst : f32) outs(%13 : memref<4x16x16x1xf32>)
    %14 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %13, %14 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %13 : memref<4x16x16x1xf32>
    linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : i32tensor<2xi64>} ins(%10, %12 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%14 : memref<4x16x16x1xf32>)
    memref.dealloc %12 : memref<5x5x1x1xf32>
    memref.dealloc %10 : memref<4x35x35x1xf32>
    %15 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map3, #map2, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %14 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%15 : memref<4x16x16x1xf32>) {
    ^bb0(%arg2: f32, %arg3: f32, %arg4: f32):
      %42 = arith.addf %arg2, %arg3 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %14 : memref<4x16x16x1xf32>
    %16 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map2, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%15 : memref<4x16x16x1xf32>) outs(%16 : memref<4x16x16x1xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      %42 = arith.cmpf olt, %arg2, %cst : f32
      %43 = arith.select %42, %cst, %arg2 : f32
      %44 = arith.cmpf olt, %cst_0, %arg2 : f32
      %45 = arith.select %44, %cst_0, %43 : f32
      linalg.yield %45 : f32
    }
    memref.dealloc %15 : memref<4x16x16x1xf32>
    %17 = memref.collapse_shape %16 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %18 = memref.expand_shape %17 [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %19 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    linalg.fill ins(%cst : f32) outs(%19 : memref<1x4x8xf32>)
    %20 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %19, %20 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %19 : memref<1x4x8xf32>
    linalg.batch_matmul ins(%18, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%20 : memref<1x4x8xf32>)
    memref.dealloc %16 : memref<4x16x16x1xf32>
    %21 = memref.collapse_shape %20 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %22 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map4, #map5, #map4], iterator_types = ["parallel", "parallel"]} ins(%21, %2 : memref<4x8xf32>, memref<8xf32>) outs(%22 : memref<4x8xf32>) {
    ^bb0(%arg2: f32, %arg3: f32, %arg4: f32):
      %42 = arith.addf %arg2, %arg3 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %20 : memref<1x4x8xf32>
    %23 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map4, #map4], iterator_types = ["parallel", "parallel"]} ins(%22 : memref<4x8xf32>) outs(%23 : memref<4x8xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      %42 = arith.cmpf olt, %arg2, %cst : f32
      %43 = arith.select %42, %cst, %arg2 : f32
      %44 = arith.cmpf olt, %cst_0, %arg2 : f32
      %45 = arith.select %44, %cst_0, %43 : f32
      linalg.yield %45 : f32
    }
    memref.dealloc %22 : memref<4x8xf32>
    %24 = memref.expand_shape %23 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %25 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    linalg.fill ins(%cst : f32) outs(%25 : memref<1x4x4xf32>)
    %26 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %25, %26 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %25 : memref<1x4x4xf32>
    linalg.batch_matmul ins(%24, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%26 : memref<1x4x4xf32>)
    memref.dealloc %23 : memref<4x8xf32>
    %27 = memref.collapse_shape %26 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %28 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map4, #map5, #map4], iterator_types = ["parallel", "parallel"]} ins(%27, %1 : memref<4x4xf32>, memref<4xf32>) outs(%28 : memref<4x4xf32>) {
    ^bb0(%arg2: f32, %arg3: f32, %arg4: f32):
      %42 = arith.addf %arg2, %arg3 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %26 : memref<1x4x4xf32>
    %29 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map4, #map4], iterator_types = ["parallel", "parallel"]} ins(%28 : memref<4x4xf32>) outs(%29 : memref<4x4xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      %42 = arith.cmpf olt, %arg2, %cst : f32
      %43 = arith.select %42, %cst, %arg2 : f32
      %44 = arith.cmpf olt, %cst_0, %arg2 : f32
      %45 = arith.select %44, %cst_0, %43 : f32
      linalg.yield %45 : f32
    }
    memref.dealloc %28 : memref<4x4xf32>
    %30 = memref.expand_shape %29 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %31 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    linalg.fill ins(%cst : f32) outs(%31 : memref<1x4x2xf32>)
    %32 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %31, %32 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %31 : memref<1x4x2xf32>
    linalg.batch_matmul ins(%30, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%32 : memref<1x4x2xf32>)
    memref.dealloc %29 : memref<4x4xf32>
    %33 = memref.collapse_shape %32 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %34 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map4, #map5, #map4], iterator_types = ["parallel", "parallel"]} ins(%33, %0 : memref<4x2xf32>, memref<2xf32>) outs(%34 : memref<4x2xf32>) {
    ^bb0(%arg2: f32, %arg3: f32, %arg4: f32):
      %42 = arith.addf %arg2, %arg3 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %32 : memref<1x4x2xf32>
    %35 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map4, #map4], iterator_types = ["parallel", "parallel"]} ins(%34 : memref<4x2xf32>) outs(%35 : memref<4x2xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      %42 = math.exp %arg2 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %34 : memref<4x2xf32>
    %36 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    linalg.fill ins(%cst : f32) outs(%36 : memref<4xf32>)
    %37 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %36, %37 : memref<4xf32> to memref<4xf32>
    memref.dealloc %36 : memref<4xf32>
    linalg.generic {indexing_maps = [#map4, #map6], iterator_types = ["parallel", "reduction"]} ins(%35 : memref<4x2xf32>) outs(%37 : memref<4xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      %42 = arith.addf %arg2, %arg3 : f32
      linalg.yield %42 : f32
    }
    %38 = memref.expand_shape %37 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %39 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    linalg.generic {indexing_maps = [#map4, #map4], iterator_types = ["parallel", "parallel"]} ins(%38 : memref<4x1xf32>) outs(%39 : memref<4x1xf32>) {
    ^bb0(%arg2: f32, %arg3: f32):
      %42 = arith.divf %cst_1, %arg2 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %37 : memref<4xf32>
    %40 = memref.collapse_shape %39 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %41 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map4, #map6, #map4], iterator_types = ["parallel", "parallel"]} ins(%35, %40 : memref<4x2xf32>, memref<4xf32>) outs(%41 : memref<4x2xf32>) {
    ^bb0(%arg2: f32, %arg3: f32, %arg4: f32):
      %42 = arith.mulf %arg2, %arg3 : f32
      linalg.yield %42 : f32
    }
    memref.dealloc %39 : memref<4x1xf32>
    memref.dealloc %35 : memref<4x2xf32>
    memref.copy %41, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
}

