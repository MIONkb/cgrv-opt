// -----// IR Dump After EmptyTensorToAllocTensor (empty-tensor-to-alloc-tensor) //----- //
#map = affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>
#map1 = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
#map2 = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
#map3 = affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>
#map4 = affine_map<(d0, d1, d2, d3) -> (d3)>
#map5 = affine_map<(d0, d1) -> (d0, d1)>
#map6 = affine_map<(d0, d1) -> (d1)>
#map7 = affine_map<(d0, d1) -> (d0)>
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    %cst_1 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
    %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
    %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
    %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
    %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
    %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
    %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map2>
    memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map2>
    %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    linalg.generic {indexing_maps = [#map3, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    memref.dealloc %alloc : memref<1x5x5x1xf32>
    %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
    %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
    linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
    memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
    memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
    %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map4, #map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %8 = arith.addf %in, %in_32 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
    %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
    %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
    %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %alloc_9 : memref<1x4x8xf32>
    linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
    memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
    %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %8 = arith.addf %in, %in_32 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_10 : memref<1x4x8xf32>
    %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_12 : memref<4x8xf32>
    %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
    %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %alloc_15 : memref<1x4x4xf32>
    linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
    memref.dealloc %alloc_13 : memref<4x8xf32>
    %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %8 = arith.addf %in, %in_32 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_16 : memref<1x4x4xf32>
    %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_18 : memref<4x4xf32>
    %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
    %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %alloc_21 : memref<1x4x2xf32>
    linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
    memref.dealloc %alloc_19 : memref<4x4xf32>
    %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %8 = arith.addf %in, %in_32 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_22 : memref<1x4x2xf32>
    %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = math.exp %in : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_24 : memref<4x2xf32>
    %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
    %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
    memref.dealloc %alloc_26 : memref<4xf32>
    linalg.generic {indexing_maps = [#map5, #map7], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.addf %in, %out : f32
      linalg.yield %8 : f32
    }
    %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.divf %cst_1, %in : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_27 : memref<4xf32>
    %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %alloc_31 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map7, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloc_31 : memref<4x2xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %8 = arith.mulf %in, %in_32 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_29 : memref<4x1xf32>
    memref.dealloc %alloc_25 : memref<4x2xf32>
    memref.copy %alloc_31, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
}


// -----// IR Dump After PromoteBuffersToStack (promote-buffers-to-stack) //----- //
func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_0 = arith.constant 3.40282347E+38 : f32
  %cst_1 = arith.constant 1.000000e+00 : f32
  %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
  %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
  %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
  %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
  %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
  %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
  %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
  %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
  %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
  %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
  memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
  %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  memref.dealloc %alloc : memref<1x5x5x1xf32>
  %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
  %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
  memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
  linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
  memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
  memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
  %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
  %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
  %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
  %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
  %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
  %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
  memref.dealloc %alloc_9 : memref<1x4x8xf32>
  linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
  memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
  %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
  %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_10 : memref<1x4x8xf32>
  %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_12 : memref<4x8xf32>
  %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
  %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
  %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
  memref.dealloc %alloc_15 : memref<1x4x4xf32>
  linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
  memref.dealloc %alloc_13 : memref<4x8xf32>
  %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
  %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_16 : memref<1x4x4xf32>
  %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_18 : memref<4x4xf32>
  %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
  %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
  %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
  memref.dealloc %alloc_21 : memref<1x4x2xf32>
  linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
  memref.dealloc %alloc_19 : memref<4x4xf32>
  %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
  %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_22 : memref<1x4x2xf32>
  %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = math.exp %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_24 : memref<4x2xf32>
  %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
  %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
  memref.dealloc %alloc_26 : memref<4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.addf %in, %out : f32
    linalg.yield %8 : f32
  }
  %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
  %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.divf %cst_1, %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_27 : memref<4xf32>
  %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
  %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloca : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.mulf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_29 : memref<4x1xf32>
  memref.dealloc %alloc_25 : memref<4x2xf32>
  memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
  return
}

// -----// IR Dump After LinalgBufferize (linalg-bufferize) //----- //
func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_0 = arith.constant 3.40282347E+38 : f32
  %cst_1 = arith.constant 1.000000e+00 : f32
  %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
  %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
  %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
  %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
  %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
  %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
  %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
  %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
  %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
  %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
  memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
  %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  memref.dealloc %alloc : memref<1x5x5x1xf32>
  %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
  %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
  memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
  linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
  memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
  memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
  %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
  %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
  %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
  %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
  %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
  %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
  memref.dealloc %alloc_9 : memref<1x4x8xf32>
  linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
  memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
  %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
  %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_10 : memref<1x4x8xf32>
  %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_12 : memref<4x8xf32>
  %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
  %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
  %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
  memref.dealloc %alloc_15 : memref<1x4x4xf32>
  linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
  memref.dealloc %alloc_13 : memref<4x8xf32>
  %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
  %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_16 : memref<1x4x4xf32>
  %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_18 : memref<4x4xf32>
  %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
  %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
  %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
  memref.dealloc %alloc_21 : memref<1x4x2xf32>
  linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
  memref.dealloc %alloc_19 : memref<4x4xf32>
  %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
  %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_22 : memref<1x4x2xf32>
  %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = math.exp %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_24 : memref<4x2xf32>
  %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
  %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
  memref.dealloc %alloc_26 : memref<4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.addf %in, %out : f32
    linalg.yield %8 : f32
  }
  %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
  %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.divf %cst_1, %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_27 : memref<4xf32>
  %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
  %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloca : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.mulf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_29 : memref<4x1xf32>
  memref.dealloc %alloc_25 : memref<4x2xf32>
  memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
  return
}

// -----// IR Dump After TensorBufferize (tensor-bufferize) //----- //
func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_0 = arith.constant 3.40282347E+38 : f32
  %cst_1 = arith.constant 1.000000e+00 : f32
  %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
  %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
  %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
  %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
  %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
  %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
  %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
  %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
  %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
  %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
  memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
  %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  memref.dealloc %alloc : memref<1x5x5x1xf32>
  %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
  %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
  memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
  linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
  memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
  memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
  %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
  %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
  %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
  %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
  %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
  %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
  memref.dealloc %alloc_9 : memref<1x4x8xf32>
  linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
  memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
  %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
  %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_10 : memref<1x4x8xf32>
  %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_12 : memref<4x8xf32>
  %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
  %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
  %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
  memref.dealloc %alloc_15 : memref<1x4x4xf32>
  linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
  memref.dealloc %alloc_13 : memref<4x8xf32>
  %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
  %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_16 : memref<1x4x4xf32>
  %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_18 : memref<4x4xf32>
  %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
  %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
  %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
  memref.dealloc %alloc_21 : memref<1x4x2xf32>
  linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
  memref.dealloc %alloc_19 : memref<4x4xf32>
  %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
  %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_22 : memref<1x4x2xf32>
  %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = math.exp %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_24 : memref<4x2xf32>
  %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
  %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
  memref.dealloc %alloc_26 : memref<4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.addf %in, %out : f32
    linalg.yield %8 : f32
  }
  %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
  %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.divf %cst_1, %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_27 : memref<4xf32>
  %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
  %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloca : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.mulf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_29 : memref<4x1xf32>
  memref.dealloc %alloc_25 : memref<4x2xf32>
  memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
  return
}

// -----// IR Dump After FuncBufferize (func-bufferize) //----- //
#map = affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>
#map1 = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
#map2 = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
#map3 = affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>
#map4 = affine_map<(d0, d1, d2, d3) -> (d3)>
#map5 = affine_map<(d0, d1) -> (d0, d1)>
#map6 = affine_map<(d0, d1) -> (d1)>
#map7 = affine_map<(d0, d1) -> (d0)>
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    %cst_1 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
    %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
    %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
    %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
    %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
    %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
    %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map2>
    memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map2>
    %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    linalg.generic {indexing_maps = [#map3, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    memref.dealloc %alloc : memref<1x5x5x1xf32>
    %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
    %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
    linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
    memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
    memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
    %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map4, #map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
    %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
    %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
    %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %alloc_9 : memref<1x4x8xf32>
    linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
    memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
    %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_10 : memref<1x4x8xf32>
    %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_12 : memref<4x8xf32>
    %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
    %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %alloc_15 : memref<1x4x4xf32>
    linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
    memref.dealloc %alloc_13 : memref<4x8xf32>
    %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_16 : memref<1x4x4xf32>
    %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_18 : memref<4x4xf32>
    %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
    %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %alloc_21 : memref<1x4x2xf32>
    linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
    memref.dealloc %alloc_19 : memref<4x4xf32>
    %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_22 : memref<1x4x2xf32>
    %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = math.exp %in : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_24 : memref<4x2xf32>
    %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
    %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
    memref.dealloc %alloc_26 : memref<4xf32>
    linalg.generic {indexing_maps = [#map5, #map7], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.addf %in, %out : f32
      linalg.yield %8 : f32
    }
    %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.divf %cst_1, %in : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_27 : memref<4xf32>
    %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map7, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloca : memref<4x2xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.mulf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_29 : memref<4x1xf32>
    memref.dealloc %alloc_25 : memref<4x2xf32>
    memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
}


// -----// IR Dump After ArithBufferize (arith-bufferize) //----- //
#map = affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>
#map1 = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
#map2 = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
#map3 = affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>
#map4 = affine_map<(d0, d1, d2, d3) -> (d3)>
#map5 = affine_map<(d0, d1) -> (d0, d1)>
#map6 = affine_map<(d0, d1) -> (d1)>
#map7 = affine_map<(d0, d1) -> (d0)>
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    %cst_1 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
    %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
    %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
    %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
    %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
    %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
    %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map2>
    memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map2>
    %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    linalg.generic {indexing_maps = [#map3, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    memref.dealloc %alloc : memref<1x5x5x1xf32>
    %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
    %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
    linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
    memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
    memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
    %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map4, #map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
    %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
    %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
    %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %alloc_9 : memref<1x4x8xf32>
    linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
    memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
    %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_10 : memref<1x4x8xf32>
    %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_12 : memref<4x8xf32>
    %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
    %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %alloc_15 : memref<1x4x4xf32>
    linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
    memref.dealloc %alloc_13 : memref<4x8xf32>
    %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_16 : memref<1x4x4xf32>
    %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.cmpf olt, %in, %cst : f32
      %9 = arith.select %8, %cst, %in : f32
      %10 = arith.cmpf olt, %cst_0, %in : f32
      %11 = arith.select %10, %cst_0, %9 : f32
      linalg.yield %11 : f32
    }
    memref.dealloc %alloc_18 : memref<4x4xf32>
    %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
    %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %alloc_21 : memref<1x4x2xf32>
    linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
    memref.dealloc %alloc_19 : memref<4x4xf32>
    %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.addf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_22 : memref<1x4x2xf32>
    %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = math.exp %in : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_24 : memref<4x2xf32>
    %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
    %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
    memref.dealloc %alloc_26 : memref<4xf32>
    linalg.generic {indexing_maps = [#map5, #map7], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.addf %in, %out : f32
      linalg.yield %8 : f32
    }
    %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %8 = arith.divf %cst_1, %in : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_27 : memref<4xf32>
    %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
    linalg.generic {indexing_maps = [#map5, #map7, #map5], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloca : memref<4x2xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %8 = arith.mulf %in, %in_31 : f32
      linalg.yield %8 : f32
    }
    memref.dealloc %alloc_29 : memref<4x1xf32>
    memref.dealloc %alloc_25 : memref<4x2xf32>
    memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
}


// -----// IR Dump After BufferizationBufferize (bufferization-bufferize) //----- //
func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_0 = arith.constant 3.40282347E+38 : f32
  %cst_1 = arith.constant 1.000000e+00 : f32
  %0 = memref.get_global @__constant_2xf32 : memref<2xf32>
  %1 = memref.get_global @__constant_4xf32 : memref<4xf32>
  %2 = memref.get_global @__constant_8xf32 : memref<8xf32>
  %3 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
  %4 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
  %5 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
  %6 = memref.get_global @__constant_1xf32 : memref<1xf32>
  %7 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
  %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d1, d2, d3, d0)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7 : memref<5x5x1x1xf32>) outs(%alloc : memref<1x5x5x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_2 : memref<4x35x35x1xf32>)
  %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
  memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
  %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc : memref<1x5x5x1xf32>) outs(%alloc_4 : memref<5x5x1x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    linalg.yield %in : f32
  }
  memref.dealloc %alloc : memref<1x5x5x1xf32>
  %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_5 : memref<4x16x16x1xf32>)
  %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
  memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
  linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, strides = dense<2> : tensor<2xi64>} ins(%alloc_3, %alloc_4 : memref<4x35x35x1xf32>, memref<5x5x1x1xf32>) outs(%alloc_6 : memref<4x16x16x1xf32>)
  memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
  memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
  %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%6, %alloc_6 : memref<1xf32>, memref<4x16x16x1xf32>) outs(%alloc_7 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
  %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%alloc_7 : memref<4x16x16x1xf32>) outs(%alloc_8 : memref<4x16x16x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
  %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
  %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
  %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_9 : memref<1x4x8xf32>)
  %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
  memref.dealloc %alloc_9 : memref<1x4x8xf32>
  linalg.batch_matmul ins(%expand_shape, %5 : memref<1x4x256xf32>, memref<1x256x8xf32>) outs(%alloc_10 : memref<1x4x8xf32>)
  memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
  %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
  %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_11, %2 : memref<4x8xf32>, memref<8xf32>) outs(%alloc_12 : memref<4x8xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_10 : memref<1x4x8xf32>
  %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_12 : memref<4x8xf32>) outs(%alloc_13 : memref<4x8xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_12 : memref<4x8xf32>
  %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
  %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_15 : memref<1x4x4xf32>)
  %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
  memref.dealloc %alloc_15 : memref<1x4x4xf32>
  linalg.batch_matmul ins(%expand_shape_14, %4 : memref<1x4x8xf32>, memref<1x8x4xf32>) outs(%alloc_16 : memref<1x4x4xf32>)
  memref.dealloc %alloc_13 : memref<4x8xf32>
  %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
  %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_17, %1 : memref<4x4xf32>, memref<4xf32>) outs(%alloc_18 : memref<4x4xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_16 : memref<1x4x4xf32>
  %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_18 : memref<4x4xf32>) outs(%alloc_19 : memref<4x4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.cmpf olt, %in, %cst : f32
    %9 = arith.select %8, %cst, %in : f32
    %10 = arith.cmpf olt, %cst_0, %in : f32
    %11 = arith.select %10, %cst_0, %9 : f32
    linalg.yield %11 : f32
  }
  memref.dealloc %alloc_18 : memref<4x4xf32>
  %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
  %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_21 : memref<1x4x2xf32>)
  %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
  memref.dealloc %alloc_21 : memref<1x4x2xf32>
  linalg.batch_matmul ins(%expand_shape_20, %3 : memref<1x4x4xf32>, memref<1x4x2xf32>) outs(%alloc_22 : memref<1x4x2xf32>)
  memref.dealloc %alloc_19 : memref<4x4xf32>
  %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
  %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%collapse_shape_23, %0 : memref<4x2xf32>, memref<2xf32>) outs(%alloc_24 : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.addf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_22 : memref<1x4x2xf32>
  %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_24 : memref<4x2xf32>) outs(%alloc_25 : memref<4x2xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = math.exp %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_24 : memref<4x2xf32>
  %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  linalg.fill ins(%cst : f32) outs(%alloc_26 : memref<4xf32>)
  %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
  memref.dealloc %alloc_26 : memref<4xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%alloc_25 : memref<4x2xf32>) outs(%alloc_27 : memref<4xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.addf %in, %out : f32
    linalg.yield %8 : f32
  }
  %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
  %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%expand_shape_28 : memref<4x1xf32>) outs(%alloc_29 : memref<4x1xf32>) {
  ^bb0(%in: f32, %out: f32):
    %8 = arith.divf %cst_1, %in : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_27 : memref<4xf32>
  %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
  %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
  linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%alloc_25, %collapse_shape_30 : memref<4x2xf32>, memref<4xf32>) outs(%alloca : memref<4x2xf32>) {
  ^bb0(%in: f32, %in_31: f32, %out: f32):
    %8 = arith.mulf %in, %in_31 : f32
    linalg.yield %8 : f32
  }
  memref.dealloc %alloc_29 : memref<4x1xf32>
  memref.dealloc %alloc_25 : memref<4x2xf32>
  memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
  return
}

// -----// IR Dump After LinalgLowerToAffineLoops (convert-linalg-to-affine-loops) //----- //
func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_0 = arith.constant 3.40282347E+38 : f32
  %cst_1 = arith.constant 1.000000e+00 : f32
  %0 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
  %1 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
  %2 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
  %3 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
  %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 5 {
      affine.for %arg4 = 0 to 5 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %3[%arg3, %arg4, %arg5, %arg2] : memref<5x5x1x1xf32>
          affine.store %4, %alloc[%arg2, %arg3, %arg4, %arg5] : memref<1x5x5x1xf32>
        }
      }
    }
  }
  %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 35 {
      affine.for %arg4 = 0 to 35 {
        affine.for %arg5 = 0 to 1 {
          affine.store %cst, %alloc_2[%arg2, %arg3, %arg4, %arg5] : memref<4x35x35x1xf32>
        }
      }
    }
  }
  %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
  memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
  %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
  affine.for %arg2 = 0 to 5 {
    affine.for %arg3 = 0 to 5 {
      affine.for %arg4 = 0 to 1 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %alloc[%arg5, %arg2, %arg3, %arg4] : memref<1x5x5x1xf32>
          affine.store %4, %alloc_4[%arg2, %arg3, %arg4, %arg5] : memref<5x5x1x1xf32>
        }
      }
    }
  }
  memref.dealloc %alloc : memref<1x5x5x1xf32>
  %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          affine.store %cst, %alloc_5[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
        }
      }
    }
  }
  %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
  memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          affine.for %arg6 = 0 to 5 {
            affine.for %arg7 = 0 to 5 {
              affine.for %arg8 = 0 to 1 {
                %4 = affine.apply affine_map<(d0, d1) -> (d0 * 2 + d1)>(%arg3, %arg6)
                %5 = affine.apply affine_map<(d0, d1) -> (d0 * 2 + d1)>(%arg4, %arg7)
                %6 = affine.load %alloc_3[%arg2, %4, %5, %arg8] : memref<4x35x35x1xf32>
                %7 = affine.load %alloc_4[%arg6, %arg7, %arg8, %arg5] : memref<5x5x1x1xf32>
                %8 = affine.load %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
                %9 = arith.mulf %6, %7 : f32
                %10 = arith.addf %8, %9 : f32
                affine.store %10, %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
              }
            }
          }
        }
      }
    }
  }
  memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
  memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
  %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          %5 = arith.addf %4, %cst : f32
          affine.store %5, %alloc_7[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
  %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %alloc_7[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          %5 = arith.cmpf olt, %4, %cst : f32
          %6 = arith.select %5, %cst, %4 : f32
          %7 = arith.cmpf olt, %cst_0, %4 : f32
          %8 = arith.select %7, %cst_0, %6 : f32
          affine.store %8, %alloc_8[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
  %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
  %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
  %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 8 {
        affine.store %cst, %alloc_9[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
      }
    }
  }
  %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
  memref.dealloc %alloc_9 : memref<1x4x8xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 8 {
        affine.for %arg5 = 0 to 256 {
          %4 = affine.load %expand_shape[%arg2, %arg3, %arg5] : memref<1x4x256xf32>
          %5 = affine.load %2[%arg2, %arg5, %arg4] : memref<1x256x8xf32>
          %6 = affine.load %alloc_10[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
          %7 = arith.mulf %4, %5 : f32
          %8 = arith.addf %6, %7 : f32
          affine.store %8, %alloc_10[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
  %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
  %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 8 {
      %4 = affine.load %collapse_shape_11[%arg2, %arg3] : memref<4x8xf32>
      %5 = arith.addf %4, %cst : f32
      affine.store %5, %alloc_12[%arg2, %arg3] : memref<4x8xf32>
    }
  }
  memref.dealloc %alloc_10 : memref<1x4x8xf32>
  %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 8 {
      %4 = affine.load %alloc_12[%arg2, %arg3] : memref<4x8xf32>
      %5 = arith.cmpf olt, %4, %cst : f32
      %6 = arith.select %5, %cst, %4 : f32
      %7 = arith.cmpf olt, %cst_0, %4 : f32
      %8 = arith.select %7, %cst_0, %6 : f32
      affine.store %8, %alloc_13[%arg2, %arg3] : memref<4x8xf32>
    }
  }
  memref.dealloc %alloc_12 : memref<4x8xf32>
  %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
  %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 4 {
        affine.store %cst, %alloc_15[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
      }
    }
  }
  %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
  memref.dealloc %alloc_15 : memref<1x4x4xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 4 {
        affine.for %arg5 = 0 to 8 {
          %4 = affine.load %expand_shape_14[%arg2, %arg3, %arg5] : memref<1x4x8xf32>
          %5 = affine.load %1[%arg2, %arg5, %arg4] : memref<1x8x4xf32>
          %6 = affine.load %alloc_16[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
          %7 = arith.mulf %4, %5 : f32
          %8 = arith.addf %6, %7 : f32
          affine.store %8, %alloc_16[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_13 : memref<4x8xf32>
  %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
  %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 4 {
      %4 = affine.load %collapse_shape_17[%arg2, %arg3] : memref<4x4xf32>
      %5 = arith.addf %4, %cst : f32
      affine.store %5, %alloc_18[%arg2, %arg3] : memref<4x4xf32>
    }
  }
  memref.dealloc %alloc_16 : memref<1x4x4xf32>
  %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 4 {
      %4 = affine.load %alloc_18[%arg2, %arg3] : memref<4x4xf32>
      %5 = arith.cmpf olt, %4, %cst : f32
      %6 = arith.select %5, %cst, %4 : f32
      %7 = arith.cmpf olt, %cst_0, %4 : f32
      %8 = arith.select %7, %cst_0, %6 : f32
      affine.store %8, %alloc_19[%arg2, %arg3] : memref<4x4xf32>
    }
  }
  memref.dealloc %alloc_18 : memref<4x4xf32>
  %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
  %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 2 {
        affine.store %cst, %alloc_21[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
      }
    }
  }
  %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
  memref.dealloc %alloc_21 : memref<1x4x2xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 2 {
        affine.for %arg5 = 0 to 4 {
          %4 = affine.load %expand_shape_20[%arg2, %arg3, %arg5] : memref<1x4x4xf32>
          %5 = affine.load %0[%arg2, %arg5, %arg4] : memref<1x4x2xf32>
          %6 = affine.load %alloc_22[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
          %7 = arith.mulf %4, %5 : f32
          %8 = arith.addf %6, %7 : f32
          affine.store %8, %alloc_22[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_19 : memref<4x4xf32>
  %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
  %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %collapse_shape_23[%arg2, %arg3] : memref<4x2xf32>
      %5 = arith.addf %4, %cst : f32
      affine.store %5, %alloc_24[%arg2, %arg3] : memref<4x2xf32>
    }
  }
  memref.dealloc %alloc_22 : memref<1x4x2xf32>
  %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %alloc_24[%arg2, %arg3] : memref<4x2xf32>
      %5 = math.exp %4 : f32
      affine.store %5, %alloc_25[%arg2, %arg3] : memref<4x2xf32>
    }
  }
  memref.dealloc %alloc_24 : memref<4x2xf32>
  %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.store %cst, %alloc_26[%arg2] : memref<4xf32>
  }
  %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
  memref.dealloc %alloc_26 : memref<4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %alloc_25[%arg2, %arg3] : memref<4x2xf32>
      %5 = affine.load %alloc_27[%arg2] : memref<4xf32>
      %6 = arith.addf %4, %5 : f32
      affine.store %6, %alloc_27[%arg2] : memref<4xf32>
    }
  }
  %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
  %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 1 {
      %4 = affine.load %expand_shape_28[%arg2, %arg3] : memref<4x1xf32>
      %5 = arith.divf %cst_1, %4 : f32
      affine.store %5, %alloc_29[%arg2, %arg3] : memref<4x1xf32>
    }
  }
  memref.dealloc %alloc_27 : memref<4xf32>
  %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
  %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %alloc_25[%arg2, %arg3] : memref<4x2xf32>
      %5 = affine.load %collapse_shape_30[%arg2] : memref<4xf32>
      %6 = arith.mulf %4, %5 : f32
      affine.store %6, %alloca[%arg2, %arg3] : memref<4x2xf32>
    }
  }
  memref.dealloc %alloc_29 : memref<4x1xf32>
  memref.dealloc %alloc_25 : memref<4x2xf32>
  memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
  return
}

// -----// IR Dump After PromoteBuffersToStack (promote-buffers-to-stack) //----- //
func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_0 = arith.constant 3.40282347E+38 : f32
  %cst_1 = arith.constant 1.000000e+00 : f32
  %0 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
  %1 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
  %2 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
  %3 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
  %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 5 {
      affine.for %arg4 = 0 to 5 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %3[%arg3, %arg4, %arg5, %arg2] : memref<5x5x1x1xf32>
          affine.store %4, %alloc[%arg2, %arg3, %arg4, %arg5] : memref<1x5x5x1xf32>
        }
      }
    }
  }
  %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 35 {
      affine.for %arg4 = 0 to 35 {
        affine.for %arg5 = 0 to 1 {
          affine.store %cst, %alloc_2[%arg2, %arg3, %arg4, %arg5] : memref<4x35x35x1xf32>
        }
      }
    }
  }
  %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
  memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
  memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
  %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>>
  %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
  affine.for %arg2 = 0 to 5 {
    affine.for %arg3 = 0 to 5 {
      affine.for %arg4 = 0 to 1 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %alloc[%arg5, %arg2, %arg3, %arg4] : memref<1x5x5x1xf32>
          affine.store %4, %alloc_4[%arg2, %arg3, %arg4, %arg5] : memref<5x5x1x1xf32>
        }
      }
    }
  }
  memref.dealloc %alloc : memref<1x5x5x1xf32>
  %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          affine.store %cst, %alloc_5[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
        }
      }
    }
  }
  %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
  memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          affine.for %arg6 = 0 to 5 {
            affine.for %arg7 = 0 to 5 {
              affine.for %arg8 = 0 to 1 {
                %4 = affine.apply affine_map<(d0, d1) -> (d0 * 2 + d1)>(%arg3, %arg6)
                %5 = affine.apply affine_map<(d0, d1) -> (d0 * 2 + d1)>(%arg4, %arg7)
                %6 = affine.load %alloc_3[%arg2, %4, %5, %arg8] : memref<4x35x35x1xf32>
                %7 = affine.load %alloc_4[%arg6, %arg7, %arg8, %arg5] : memref<5x5x1x1xf32>
                %8 = affine.load %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
                %9 = arith.mulf %6, %7 : f32
                %10 = arith.addf %8, %9 : f32
                affine.store %10, %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
              }
            }
          }
        }
      }
    }
  }
  memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
  memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
  %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          %5 = arith.addf %4, %cst : f32
          affine.store %5, %alloc_7[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
  %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 16 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 1 {
          %4 = affine.load %alloc_7[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          %5 = arith.cmpf olt, %4, %cst : f32
          %6 = arith.select %5, %cst, %4 : f32
          %7 = arith.cmpf olt, %cst_0, %4 : f32
          %8 = arith.select %7, %cst_0, %6 : f32
          affine.store %8, %alloc_8[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
  %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
  %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
  %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 8 {
        affine.store %cst, %alloc_9[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
      }
    }
  }
  %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
  memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
  memref.dealloc %alloc_9 : memref<1x4x8xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 8 {
        affine.for %arg5 = 0 to 256 {
          %4 = affine.load %expand_shape[%arg2, %arg3, %arg5] : memref<1x4x256xf32>
          %5 = affine.load %2[%arg2, %arg5, %arg4] : memref<1x256x8xf32>
          %6 = affine.load %alloc_10[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
          %7 = arith.mulf %4, %5 : f32
          %8 = arith.addf %6, %7 : f32
          affine.store %8, %alloc_10[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
  %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
  %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 8 {
      %4 = affine.load %collapse_shape_11[%arg2, %arg3] : memref<4x8xf32>
      %5 = arith.addf %4, %cst : f32
      affine.store %5, %alloc_12[%arg2, %arg3] : memref<4x8xf32>
    }
  }
  memref.dealloc %alloc_10 : memref<1x4x8xf32>
  %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 8 {
      %4 = affine.load %alloc_12[%arg2, %arg3] : memref<4x8xf32>
      %5 = arith.cmpf olt, %4, %cst : f32
      %6 = arith.select %5, %cst, %4 : f32
      %7 = arith.cmpf olt, %cst_0, %4 : f32
      %8 = arith.select %7, %cst_0, %6 : f32
      affine.store %8, %alloc_13[%arg2, %arg3] : memref<4x8xf32>
    }
  }
  memref.dealloc %alloc_12 : memref<4x8xf32>
  %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
  %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 4 {
        affine.store %cst, %alloc_15[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
      }
    }
  }
  %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
  memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
  memref.dealloc %alloc_15 : memref<1x4x4xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 4 {
        affine.for %arg5 = 0 to 8 {
          %4 = affine.load %expand_shape_14[%arg2, %arg3, %arg5] : memref<1x4x8xf32>
          %5 = affine.load %1[%arg2, %arg5, %arg4] : memref<1x8x4xf32>
          %6 = affine.load %alloc_16[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
          %7 = arith.mulf %4, %5 : f32
          %8 = arith.addf %6, %7 : f32
          affine.store %8, %alloc_16[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_13 : memref<4x8xf32>
  %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
  %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 4 {
      %4 = affine.load %collapse_shape_17[%arg2, %arg3] : memref<4x4xf32>
      %5 = arith.addf %4, %cst : f32
      affine.store %5, %alloc_18[%arg2, %arg3] : memref<4x4xf32>
    }
  }
  memref.dealloc %alloc_16 : memref<1x4x4xf32>
  %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 4 {
      %4 = affine.load %alloc_18[%arg2, %arg3] : memref<4x4xf32>
      %5 = arith.cmpf olt, %4, %cst : f32
      %6 = arith.select %5, %cst, %4 : f32
      %7 = arith.cmpf olt, %cst_0, %4 : f32
      %8 = arith.select %7, %cst_0, %6 : f32
      affine.store %8, %alloc_19[%arg2, %arg3] : memref<4x4xf32>
    }
  }
  memref.dealloc %alloc_18 : memref<4x4xf32>
  %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
  %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 2 {
        affine.store %cst, %alloc_21[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
      }
    }
  }
  %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
  memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
  memref.dealloc %alloc_21 : memref<1x4x2xf32>
  affine.for %arg2 = 0 to 1 {
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 2 {
        affine.for %arg5 = 0 to 4 {
          %4 = affine.load %expand_shape_20[%arg2, %arg3, %arg5] : memref<1x4x4xf32>
          %5 = affine.load %0[%arg2, %arg5, %arg4] : memref<1x4x2xf32>
          %6 = affine.load %alloc_22[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
          %7 = arith.mulf %4, %5 : f32
          %8 = arith.addf %6, %7 : f32
          affine.store %8, %alloc_22[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
        }
      }
    }
  }
  memref.dealloc %alloc_19 : memref<4x4xf32>
  %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
  %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %collapse_shape_23[%arg2, %arg3] : memref<4x2xf32>
      %5 = arith.addf %4, %cst : f32
      affine.store %5, %alloc_24[%arg2, %arg3] : memref<4x2xf32>
    }
  }
  memref.dealloc %alloc_22 : memref<1x4x2xf32>
  %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %alloc_24[%arg2, %arg3] : memref<4x2xf32>
      %5 = math.exp %4 : f32
      affine.store %5, %alloc_25[%arg2, %arg3] : memref<4x2xf32>
    }
  }
  memref.dealloc %alloc_24 : memref<4x2xf32>
  %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.store %cst, %alloc_26[%arg2] : memref<4xf32>
  }
  %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
  memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
  memref.dealloc %alloc_26 : memref<4xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %alloc_25[%arg2, %arg3] : memref<4x2xf32>
      %5 = affine.load %alloc_27[%arg2] : memref<4xf32>
      %6 = arith.addf %4, %5 : f32
      affine.store %6, %alloc_27[%arg2] : memref<4xf32>
    }
  }
  %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
  %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 1 {
      %4 = affine.load %expand_shape_28[%arg2, %arg3] : memref<4x1xf32>
      %5 = arith.divf %cst_1, %4 : f32
      affine.store %5, %alloc_29[%arg2, %arg3] : memref<4x1xf32>
    }
  }
  memref.dealloc %alloc_27 : memref<4xf32>
  %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
  %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
  affine.for %arg2 = 0 to 4 {
    affine.for %arg3 = 0 to 2 {
      %4 = affine.load %alloc_25[%arg2, %arg3] : memref<4x2xf32>
      %5 = affine.load %collapse_shape_30[%arg2] : memref<4xf32>
      %6 = arith.mulf %4, %5 : f32
      affine.store %6, %alloca[%arg2, %arg3] : memref<4x2xf32>
    }
  }
  memref.dealloc %alloc_29 : memref<4x1xf32>
  memref.dealloc %alloc_25 : memref<4x2xf32>
  memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
  return
}

// -----// IR Dump After Canonicalizer (canonicalize) //----- //
#map = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    %cst_1 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %1 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %2 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %3 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %alloc = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 5 {
        affine.for %arg4 = 0 to 5 {
          affine.for %arg5 = 0 to 1 {
            %4 = affine.load %3[%arg3, %arg4, %arg5, %arg2] : memref<5x5x1x1xf32>
            affine.store %4, %alloc[%arg2, %arg3, %arg4, %arg5] : memref<1x5x5x1xf32>
          }
        }
      }
    }
    %alloc_2 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 35 {
        affine.for %arg4 = 0 to 35 {
          affine.for %arg5 = 0 to 1 {
            affine.store %cst, %alloc_2[%arg2, %arg3, %arg4, %arg5] : memref<4x35x35x1xf32>
          }
        }
      }
    }
    %alloc_3 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %alloc_2, %alloc_3 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %alloc_2 : memref<4x35x35x1xf32>
    %subview = memref.subview %alloc_3[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map>
    memref.copy %arg0, %subview : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map>
    %alloc_4 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    affine.for %arg2 = 0 to 5 {
      affine.for %arg3 = 0 to 5 {
        affine.for %arg4 = 0 to 1 {
          affine.for %arg5 = 0 to 1 {
            %4 = affine.load %alloc[%arg5, %arg2, %arg3, %arg4] : memref<1x5x5x1xf32>
            affine.store %4, %alloc_4[%arg2, %arg3, %arg4, %arg5] : memref<5x5x1x1xf32>
          }
        }
      }
    }
    memref.dealloc %alloc : memref<1x5x5x1xf32>
    %alloc_5 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 16 {
        affine.for %arg4 = 0 to 16 {
          affine.for %arg5 = 0 to 1 {
            affine.store %cst, %alloc_5[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    %alloc_6 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %alloc_5, %alloc_6 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %alloc_5 : memref<4x16x16x1xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 16 {
        affine.for %arg4 = 0 to 16 {
          affine.for %arg5 = 0 to 1 {
            affine.for %arg6 = 0 to 5 {
              affine.for %arg7 = 0 to 5 {
                affine.for %arg8 = 0 to 1 {
                  %4 = affine.load %alloc_3[%arg2, %arg3 * 2 + %arg6, %arg4 * 2 + %arg7, %arg8] : memref<4x35x35x1xf32>
                  %5 = affine.load %alloc_4[%arg6, %arg7, %arg8, %arg5] : memref<5x5x1x1xf32>
                  %6 = affine.load %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
                  %7 = arith.mulf %4, %5 : f32
                  %8 = arith.addf %6, %7 : f32
                  affine.store %8, %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
                }
              }
            }
          }
        }
      }
    }
    memref.dealloc %alloc_4 : memref<5x5x1x1xf32>
    memref.dealloc %alloc_3 : memref<4x35x35x1xf32>
    %alloc_7 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 16 {
        affine.for %arg4 = 0 to 16 {
          affine.for %arg5 = 0 to 1 {
            %4 = affine.load %alloc_6[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
            %5 = arith.addf %4, %cst : f32
            affine.store %5, %alloc_7[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    memref.dealloc %alloc_6 : memref<4x16x16x1xf32>
    %alloc_8 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 16 {
        affine.for %arg4 = 0 to 16 {
          affine.for %arg5 = 0 to 1 {
            %4 = affine.load %alloc_7[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
            %5 = arith.cmpf olt, %4, %cst : f32
            %6 = arith.select %5, %cst, %4 : f32
            %7 = arith.cmpf olt, %cst_0, %4 : f32
            %8 = arith.select %7, %cst_0, %6 : f32
            affine.store %8, %alloc_8[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    memref.dealloc %alloc_7 : memref<4x16x16x1xf32>
    %collapse_shape = memref.collapse_shape %alloc_8 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %expand_shape = memref.expand_shape %collapse_shape [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %alloc_9 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 4 {
        affine.for %arg4 = 0 to 8 {
          affine.store %cst, %alloc_9[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
        }
      }
    }
    %alloc_10 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %alloc_9, %alloc_10 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %alloc_9 : memref<1x4x8xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 4 {
        affine.for %arg4 = 0 to 8 {
          affine.for %arg5 = 0 to 256 {
            %4 = affine.load %expand_shape[%arg2, %arg3, %arg5] : memref<1x4x256xf32>
            %5 = affine.load %2[%arg2, %arg5, %arg4] : memref<1x256x8xf32>
            %6 = affine.load %alloc_10[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
            %7 = arith.mulf %4, %5 : f32
            %8 = arith.addf %6, %7 : f32
            affine.store %8, %alloc_10[%arg2, %arg3, %arg4] : memref<1x4x8xf32>
          }
        }
      }
    }
    memref.dealloc %alloc_8 : memref<4x16x16x1xf32>
    %collapse_shape_11 = memref.collapse_shape %alloc_10 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %alloc_12 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 8 {
        %4 = affine.load %collapse_shape_11[%arg2, %arg3] : memref<4x8xf32>
        %5 = arith.addf %4, %cst : f32
        affine.store %5, %alloc_12[%arg2, %arg3] : memref<4x8xf32>
      }
    }
    memref.dealloc %alloc_10 : memref<1x4x8xf32>
    %alloc_13 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 8 {
        %4 = affine.load %alloc_12[%arg2, %arg3] : memref<4x8xf32>
        %5 = arith.cmpf olt, %4, %cst : f32
        %6 = arith.select %5, %cst, %4 : f32
        %7 = arith.cmpf olt, %cst_0, %4 : f32
        %8 = arith.select %7, %cst_0, %6 : f32
        affine.store %8, %alloc_13[%arg2, %arg3] : memref<4x8xf32>
      }
    }
    memref.dealloc %alloc_12 : memref<4x8xf32>
    %expand_shape_14 = memref.expand_shape %alloc_13 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %alloc_15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 4 {
        affine.for %arg4 = 0 to 4 {
          affine.store %cst, %alloc_15[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
        }
      }
    }
    %alloc_16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %alloc_15, %alloc_16 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %alloc_15 : memref<1x4x4xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 4 {
        affine.for %arg4 = 0 to 4 {
          affine.for %arg5 = 0 to 8 {
            %4 = affine.load %expand_shape_14[%arg2, %arg3, %arg5] : memref<1x4x8xf32>
            %5 = affine.load %1[%arg2, %arg5, %arg4] : memref<1x8x4xf32>
            %6 = affine.load %alloc_16[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
            %7 = arith.mulf %4, %5 : f32
            %8 = arith.addf %6, %7 : f32
            affine.store %8, %alloc_16[%arg2, %arg3, %arg4] : memref<1x4x4xf32>
          }
        }
      }
    }
    memref.dealloc %alloc_13 : memref<4x8xf32>
    %collapse_shape_17 = memref.collapse_shape %alloc_16 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %alloc_18 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 4 {
        %4 = affine.load %collapse_shape_17[%arg2, %arg3] : memref<4x4xf32>
        %5 = arith.addf %4, %cst : f32
        affine.store %5, %alloc_18[%arg2, %arg3] : memref<4x4xf32>
      }
    }
    memref.dealloc %alloc_16 : memref<1x4x4xf32>
    %alloc_19 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 4 {
        %4 = affine.load %alloc_18[%arg2, %arg3] : memref<4x4xf32>
        %5 = arith.cmpf olt, %4, %cst : f32
        %6 = arith.select %5, %cst, %4 : f32
        %7 = arith.cmpf olt, %cst_0, %4 : f32
        %8 = arith.select %7, %cst_0, %6 : f32
        affine.store %8, %alloc_19[%arg2, %arg3] : memref<4x4xf32>
      }
    }
    memref.dealloc %alloc_18 : memref<4x4xf32>
    %expand_shape_20 = memref.expand_shape %alloc_19 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %alloc_21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 4 {
        affine.for %arg4 = 0 to 2 {
          affine.store %cst, %alloc_21[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
        }
      }
    }
    %alloc_22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %alloc_21, %alloc_22 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %alloc_21 : memref<1x4x2xf32>
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 4 {
        affine.for %arg4 = 0 to 2 {
          affine.for %arg5 = 0 to 4 {
            %4 = affine.load %expand_shape_20[%arg2, %arg3, %arg5] : memref<1x4x4xf32>
            %5 = affine.load %0[%arg2, %arg5, %arg4] : memref<1x4x2xf32>
            %6 = affine.load %alloc_22[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
            %7 = arith.mulf %4, %5 : f32
            %8 = arith.addf %6, %7 : f32
            affine.store %8, %alloc_22[%arg2, %arg3, %arg4] : memref<1x4x2xf32>
          }
        }
      }
    }
    memref.dealloc %alloc_19 : memref<4x4xf32>
    %collapse_shape_23 = memref.collapse_shape %alloc_22 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %alloc_24 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %4 = affine.load %collapse_shape_23[%arg2, %arg3] : memref<4x2xf32>
        %5 = arith.addf %4, %cst : f32
        affine.store %5, %alloc_24[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    memref.dealloc %alloc_22 : memref<1x4x2xf32>
    %alloc_25 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %4 = affine.load %alloc_24[%arg2, %arg3] : memref<4x2xf32>
        %5 = math.exp %4 : f32
        affine.store %5, %alloc_25[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    memref.dealloc %alloc_24 : memref<4x2xf32>
    %alloc_26 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    affine.for %arg2 = 0 to 4 {
      affine.store %cst, %alloc_26[%arg2] : memref<4xf32>
    }
    %alloc_27 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %alloc_26, %alloc_27 : memref<4xf32> to memref<4xf32>
    memref.dealloc %alloc_26 : memref<4xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %4 = affine.load %alloc_25[%arg2, %arg3] : memref<4x2xf32>
        %5 = affine.load %alloc_27[%arg2] : memref<4xf32>
        %6 = arith.addf %4, %5 : f32
        affine.store %6, %alloc_27[%arg2] : memref<4xf32>
      }
    }
    %expand_shape_28 = memref.expand_shape %alloc_27 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %alloc_29 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 1 {
        %4 = affine.load %expand_shape_28[%arg2, %arg3] : memref<4x1xf32>
        %5 = arith.divf %cst_1, %4 : f32
        affine.store %5, %alloc_29[%arg2, %arg3] : memref<4x1xf32>
      }
    }
    memref.dealloc %alloc_27 : memref<4xf32>
    %collapse_shape_30 = memref.collapse_shape %alloc_29 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %alloca = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %4 = affine.load %alloc_25[%arg2, %arg3] : memref<4x2xf32>
        %5 = affine.load %collapse_shape_30[%arg2] : memref<4xf32>
        %6 = arith.mulf %4, %5 : f32
        affine.store %6, %alloca[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    memref.dealloc %alloc_29 : memref<4x1xf32>
    memref.dealloc %alloc_25 : memref<4x2xf32>
    memref.copy %alloca, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
}


