#map = affine_map<(d0, d1, d2, d3) -> (d0 * 1225 + d1 * 35 + d2 + d3 + 36)>
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 1087 : i32}} {
  memref.global "private" constant @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32> = dense<[[[[-0.144123852]], [[-0.203296721]], [[0.0245194137]], [[0.0424404144]], [[-5.772960e-02]]], [[[0.213269472]], [[-0.00469377637]], [[0.345114052]], [[0.136302114]], [[-0.259548783]]], [[[0.145365268]], [[0.112524837]], [[0.0500608087]], [[-0.0937015414]], [[-0.0550665557]]], [[[0.0901061296]], [[0.286698043]], [[0.111992478]], [[0.231037259]], [[-0.28824234]]], [[[-0.15258716]], [[-0.335655361]], [[0.156836212]], [[0.183970571]], [[0.124615282]]]]>
  memref.global "private" constant @__constant_1xf32 : memref<1xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_1x256x8xf32 : memref<1x256x8xf32> = dense<"">
  memref.global "private" constant @__constant_1x8x4xf32 : memref<1x8x4xf32> = dense<[[[-0.431032777, 0.339656174, -0.0172508955, 0.530783117], [-0.131361604, -0.686532557, 0.29633683, -0.190197289], [0.114347279, 0.56687206, -0.5206815, 0.0635936856], [0.278496087, -0.701272607, -0.192214727, 0.0925094485], [-0.263988972, 0.210848868, 0.702368438, -0.632420897], [0.552770555, -0.32865116, -0.0839602351, 0.335239589], [-0.466998219, -0.367347717, 0.669422566, 0.683026611], [-0.653910696, -0.016515851, 0.00850236415, 0.640155136]]]>
  memref.global "private" constant @__constant_1x4x2xf32 : memref<1x4x2xf32> = dense<[[[-0.649729251, -0.36405158], [0.711094856, -0.165482044], [-0.528160095, -0.548771381], [0.198056698, 0.657765388]]]>
  memref.global "private" constant @__constant_8xf32 : memref<8xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_4xf32 : memref<4xf32> = dense<0.000000e+00>
  memref.global "private" constant @__constant_2xf32 : memref<2xf32> = dense<0.000000e+00>
  func.func @main(%arg0: memref<4x32x32x1xf32>, %arg1: memref<4x2xf32>) attributes {tf.entry_function = {control_outputs = "", inputs = "x1", outputs = "Identity"}} {
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    %cst_1 = arith.constant 1.000000e+00 : f32
    %0 = memref.get_global @__constant_1x4x2xf32 : memref<1x4x2xf32>
    %1 = memref.get_global @__constant_1x8x4xf32 : memref<1x8x4xf32>
    %2 = memref.get_global @__constant_1x256x8xf32 : memref<1x256x8xf32>
    %3 = memref.get_global @__constant_5x5x1x1xf32 : memref<5x5x1x1xf32>
    %4 = memref.alloc() {alignment = 128 : i64} : memref<1x5x5x1xf32>
    FDRA.KernelCall @main_kernel_0(%3, %4) : (memref<5x5x1x1xf32>, memref<1x5x5x1xf32>) -> ()
    %5 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    FDRA.KernelCall @main_kernel_1(%5) : (memref<4x35x35x1xf32>) -> ()
    %6 = memref.alloc() {alignment = 128 : i64} : memref<4x35x35x1xf32>
    memref.copy %5, %6 : memref<4x35x35x1xf32> to memref<4x35x35x1xf32>
    memref.dealloc %5 : memref<4x35x35x1xf32>
    %7 = memref.subview %6[0, 1, 1, 0] [4, 32, 32, 1] [1, 1, 1, 1] : memref<4x35x35x1xf32> to memref<4x32x32x1xf32, #map>
    memref.copy %arg0, %7 : memref<4x32x32x1xf32> to memref<4x32x32x1xf32, #map>
    %8 = memref.alloc() {alignment = 128 : i64} : memref<5x5x1x1xf32>
    FDRA.KernelCall @main_kernel_2(%4, %8) : (memref<1x5x5x1xf32>, memref<5x5x1x1xf32>) -> ()
    memref.dealloc %4 : memref<1x5x5x1xf32>
    %9 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    FDRA.KernelCall @main_kernel_3(%9) : (memref<4x16x16x1xf32>) -> ()
    %10 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    memref.copy %9, %10 : memref<4x16x16x1xf32> to memref<4x16x16x1xf32>
    memref.dealloc %9 : memref<4x16x16x1xf32>
    FDRA.KernelCall @main_kernel_4(%6, %8, %10) : (memref<4x35x35x1xf32>, memref<5x5x1x1xf32>, memref<4x16x16x1xf32>) -> ()
    memref.dealloc %8 : memref<5x5x1x1xf32>
    memref.dealloc %6 : memref<4x35x35x1xf32>
    %11 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    FDRA.KernelCall @main_kernel_5(%10, %11) : (memref<4x16x16x1xf32>, memref<4x16x16x1xf32>) -> ()
    memref.dealloc %10 : memref<4x16x16x1xf32>
    %12 = memref.alloc() {alignment = 128 : i64} : memref<4x16x16x1xf32>
    FDRA.KernelCall @main_kernel_6(%11, %12) : (memref<4x16x16x1xf32>, memref<4x16x16x1xf32>) -> ()
    memref.dealloc %11 : memref<4x16x16x1xf32>
    %13 = memref.collapse_shape %12 [[0], [1, 2, 3]] : memref<4x16x16x1xf32> into memref<4x256xf32>
    %14 = memref.expand_shape %13 [[0, 1], [2]] : memref<4x256xf32> into memref<1x4x256xf32>
    %15 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    FDRA.KernelCall @main_kernel_7(%15) : (memref<1x4x8xf32>) -> ()
    %16 = memref.alloc() {alignment = 128 : i64} : memref<1x4x8xf32>
    memref.copy %15, %16 : memref<1x4x8xf32> to memref<1x4x8xf32>
    memref.dealloc %15 : memref<1x4x8xf32>
    FDRA.KernelCall @main_kernel_8(%14, %2, %16) : (memref<1x4x256xf32>, memref<1x256x8xf32>, memref<1x4x8xf32>) -> ()
    memref.dealloc %12 : memref<4x16x16x1xf32>
    %17 = memref.collapse_shape %16 [[0, 1], [2]] : memref<1x4x8xf32> into memref<4x8xf32>
    %18 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    FDRA.KernelCall @main_kernel_9(%17, %18) : (memref<4x8xf32>, memref<4x8xf32>) -> ()
    memref.dealloc %16 : memref<1x4x8xf32>
    %19 = memref.alloc() {alignment = 128 : i64} : memref<4x8xf32>
    FDRA.KernelCall @main_kernel_10(%18, %19) : (memref<4x8xf32>, memref<4x8xf32>) -> ()
    memref.dealloc %18 : memref<4x8xf32>
    %20 = memref.expand_shape %19 [[0, 1], [2]] : memref<4x8xf32> into memref<1x4x8xf32>
    %21 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    FDRA.KernelCall @main_kernel_11(%21) : (memref<1x4x4xf32>) -> ()
    %22 = memref.alloc() {alignment = 128 : i64} : memref<1x4x4xf32>
    memref.copy %21, %22 : memref<1x4x4xf32> to memref<1x4x4xf32>
    memref.dealloc %21 : memref<1x4x4xf32>
    FDRA.KernelCall @main_kernel_12(%20, %1, %22) : (memref<1x4x8xf32>, memref<1x8x4xf32>, memref<1x4x4xf32>) -> ()
    memref.dealloc %19 : memref<4x8xf32>
    %23 = memref.collapse_shape %22 [[0, 1], [2]] : memref<1x4x4xf32> into memref<4x4xf32>
    %24 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    FDRA.KernelCall @main_kernel_13(%23, %24) : (memref<4x4xf32>, memref<4x4xf32>) -> ()
    memref.dealloc %22 : memref<1x4x4xf32>
    %25 = memref.alloc() {alignment = 128 : i64} : memref<4x4xf32>
    FDRA.KernelCall @main_kernel_14(%24, %25) : (memref<4x4xf32>, memref<4x4xf32>) -> ()
    memref.dealloc %24 : memref<4x4xf32>
    %26 = memref.expand_shape %25 [[0, 1], [2]] : memref<4x4xf32> into memref<1x4x4xf32>
    %27 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    FDRA.KernelCall @main_kernel_15(%27) : (memref<1x4x2xf32>) -> ()
    %28 = memref.alloc() {alignment = 128 : i64} : memref<1x4x2xf32>
    memref.copy %27, %28 : memref<1x4x2xf32> to memref<1x4x2xf32>
    memref.dealloc %27 : memref<1x4x2xf32>
    FDRA.KernelCall @main_kernel_16(%26, %0, %28) : (memref<1x4x4xf32>, memref<1x4x2xf32>, memref<1x4x2xf32>) -> ()
    memref.dealloc %25 : memref<4x4xf32>
    %29 = memref.collapse_shape %28 [[0, 1], [2]] : memref<1x4x2xf32> into memref<4x2xf32>
    %30 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    FDRA.KernelCall @main_kernel_17(%29, %30) : (memref<4x2xf32>, memref<4x2xf32>) -> ()
    memref.dealloc %28 : memref<1x4x2xf32>
    %31 = memref.alloc() {alignment = 128 : i64} : memref<4x2xf32>
    FDRA.KernelCall @main_kernel_18(%30, %31) : (memref<4x2xf32>, memref<4x2xf32>) -> ()
    memref.dealloc %30 : memref<4x2xf32>
    %32 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    FDRA.KernelCall @main_kernel_19(%32) : (memref<4xf32>) -> ()
    %33 = memref.alloc() {alignment = 128 : i64} : memref<4xf32>
    memref.copy %32, %33 : memref<4xf32> to memref<4xf32>
    memref.dealloc %32 : memref<4xf32>
    FDRA.KernelCall @main_kernel_20(%31, %33) : (memref<4x2xf32>, memref<4xf32>) -> ()
    %34 = memref.expand_shape %33 [[0, 1]] : memref<4xf32> into memref<4x1xf32>
    %35 = memref.alloc() {alignment = 128 : i64} : memref<4x1xf32>
    FDRA.KernelCall @main_kernel_21(%34, %35) : (memref<4x1xf32>, memref<4x1xf32>) -> ()
    memref.dealloc %33 : memref<4xf32>
    %36 = memref.collapse_shape %35 [[0, 1]] : memref<4x1xf32> into memref<4xf32>
    %37 = memref.alloca() {alignment = 128 : i64} : memref<4x2xf32>
    FDRA.KernelCall @main_kernel_22(%31, %36, %37) : (memref<4x2xf32>, memref<4xf32>, memref<4x2xf32>) -> ()
    memref.dealloc %35 : memref<4x1xf32>
    memref.dealloc %31 : memref<4x2xf32>
    memref.copy %37, %arg1 : memref<4x2xf32> to memref<4x2xf32>
    return
  }
  func.func @main_kernel_0(%arg0: memref<5x5x1x1xf32>, %arg1: memref<1x5x5x1xf32>) attributes {Kernel, main_kernel_0} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg2 = 0 to 1 {
      affine.for %arg3 = 0 to 5 {
        affine.for %arg4 = 0 to 5 {
          affine.for %arg5 = 0 to 1 {
            %0 = affine.load %arg0[%arg3, %arg4, %arg5, %arg2] : memref<5x5x1x1xf32>
            affine.store %0, %arg1[%arg2, %arg3, %arg4, %arg5] : memref<1x5x5x1xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_1(%arg0: memref<4x35x35x1xf32>) attributes {Kernel, main_kernel_1} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg1 = 0 to 4 {
      affine.for %arg2 = 0 to 35 {
        affine.for %arg3 = 0 to 35 {
          affine.for %arg4 = 0 to 1 {
            affine.store %cst, %arg0[%arg1, %arg2, %arg3, %arg4] : memref<4x35x35x1xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_2(%arg0: memref<1x5x5x1xf32>, %arg1: memref<5x5x1x1xf32>) attributes {Kernel, main_kernel_2} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg2 = 0 to 5 {
      affine.for %arg3 = 0 to 5 {
        affine.for %arg4 = 0 to 1 {
          affine.for %arg5 = 0 to 1 {
            %0 = affine.load %arg0[%arg5, %arg2, %arg3, %arg4] : memref<1x5x5x1xf32>
            affine.store %0, %arg1[%arg2, %arg3, %arg4, %arg5] : memref<5x5x1x1xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_3(%arg0: memref<4x16x16x1xf32>) attributes {Kernel, main_kernel_3} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg1 = 0 to 4 {
      affine.for %arg2 = 0 to 16 {
        affine.for %arg3 = 0 to 16 {
          affine.for %arg4 = 0 to 1 {
            affine.store %cst, %arg0[%arg1, %arg2, %arg3, %arg4] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_4(%arg0: memref<4x35x35x1xf32>, %arg1: memref<5x5x1x1xf32>, %arg2: memref<4x16x16x1xf32>) attributes {Kernel, main_kernel_4} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 16 {
        affine.for %arg5 = 0 to 16 {
          affine.for %arg6 = 0 to 1 {
            affine.for %arg7 = 0 to 5 {
              affine.for %arg8 = 0 to 5 {
                affine.for %arg9 = 0 to 1 {
                  %0 = affine.load %arg0[%arg3, %arg4 * 2 + %arg7, %arg5 * 2 + %arg8, %arg9] : memref<4x35x35x1xf32>
                  %1 = affine.load %arg1[%arg7, %arg8, %arg9, %arg6] : memref<5x5x1x1xf32>
                  %2 = affine.load %arg2[%arg3, %arg4, %arg5, %arg6] : memref<4x16x16x1xf32>
                  %3 = arith.mulf %0, %1 : f32
                  %4 = arith.addf %2, %3 : f32
                  affine.store %4, %arg2[%arg3, %arg4, %arg5, %arg6] : memref<4x16x16x1xf32>
                }
              }
            }
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_5(%arg0: memref<4x16x16x1xf32>, %arg1: memref<4x16x16x1xf32>) attributes {Kernel, main_kernel_5} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 16 {
        affine.for %arg4 = 0 to 16 {
          affine.for %arg5 = 0 to 1 {
            %0 = affine.load %arg0[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
            %1 = arith.addf %0, %cst : f32
            affine.store %1, %arg1[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_6(%arg0: memref<4x16x16x1xf32>, %arg1: memref<4x16x16x1xf32>) attributes {Kernel, main_kernel_6} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 16 {
        affine.for %arg4 = 0 to 16 {
          affine.for %arg5 = 0 to 1 {
            %0 = affine.load %arg0[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
            %1 = arith.cmpf olt, %0, %cst : f32
            %2 = arith.select %1, %cst, %0 : f32
            %3 = arith.cmpf olt, %cst_0, %0 : f32
            %4 = arith.select %3, %cst_0, %2 : f32
            affine.store %4, %arg1[%arg2, %arg3, %arg4, %arg5] : memref<4x16x16x1xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_7(%arg0: memref<1x4x8xf32>) attributes {Kernel, main_kernel_7} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg1 = 0 to 1 {
      affine.for %arg2 = 0 to 4 {
        affine.for %arg3 = 0 to 8 {
          affine.store %cst, %arg0[%arg1, %arg2, %arg3] : memref<1x4x8xf32>
        }
      }
    }
    return
  }
  func.func @main_kernel_8(%arg0: memref<1x4x256xf32>, %arg1: memref<1x256x8xf32>, %arg2: memref<1x4x8xf32>) attributes {Kernel, main_kernel_8} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg3 = 0 to 1 {
      affine.for %arg4 = 0 to 4 {
        affine.for %arg5 = 0 to 8 {
          affine.for %arg6 = 0 to 256 {
            %0 = affine.load %arg0[%arg3, %arg4, %arg6] : memref<1x4x256xf32>
            %1 = affine.load %arg1[%arg3, %arg6, %arg5] : memref<1x256x8xf32>
            %2 = affine.load %arg2[%arg3, %arg4, %arg5] : memref<1x4x8xf32>
            %3 = arith.mulf %0, %1 : f32
            %4 = arith.addf %2, %3 : f32
            affine.store %4, %arg2[%arg3, %arg4, %arg5] : memref<1x4x8xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_9(%arg0: memref<4x8xf32>, %arg1: memref<4x8xf32>) attributes {Kernel, main_kernel_9} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 8 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x8xf32>
        %1 = arith.addf %0, %cst : f32
        affine.store %1, %arg1[%arg2, %arg3] : memref<4x8xf32>
      }
    }
    return
  }
  func.func @main_kernel_10(%arg0: memref<4x8xf32>, %arg1: memref<4x8xf32>) attributes {Kernel, main_kernel_10} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 8 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x8xf32>
        %1 = arith.cmpf olt, %0, %cst : f32
        %2 = arith.select %1, %cst, %0 : f32
        %3 = arith.cmpf olt, %cst_0, %0 : f32
        %4 = arith.select %3, %cst_0, %2 : f32
        affine.store %4, %arg1[%arg2, %arg3] : memref<4x8xf32>
      }
    }
    return
  }
  func.func @main_kernel_11(%arg0: memref<1x4x4xf32>) attributes {Kernel, main_kernel_11} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg1 = 0 to 1 {
      affine.for %arg2 = 0 to 4 {
        affine.for %arg3 = 0 to 4 {
          affine.store %cst, %arg0[%arg1, %arg2, %arg3] : memref<1x4x4xf32>
        }
      }
    }
    return
  }
  func.func @main_kernel_12(%arg0: memref<1x4x8xf32>, %arg1: memref<1x8x4xf32>, %arg2: memref<1x4x4xf32>) attributes {Kernel, main_kernel_12} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg3 = 0 to 1 {
      affine.for %arg4 = 0 to 4 {
        affine.for %arg5 = 0 to 4 {
          affine.for %arg6 = 0 to 8 {
            %0 = affine.load %arg0[%arg3, %arg4, %arg6] : memref<1x4x8xf32>
            %1 = affine.load %arg1[%arg3, %arg6, %arg5] : memref<1x8x4xf32>
            %2 = affine.load %arg2[%arg3, %arg4, %arg5] : memref<1x4x4xf32>
            %3 = arith.mulf %0, %1 : f32
            %4 = arith.addf %2, %3 : f32
            affine.store %4, %arg2[%arg3, %arg4, %arg5] : memref<1x4x4xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_13(%arg0: memref<4x4xf32>, %arg1: memref<4x4xf32>) attributes {Kernel, main_kernel_13} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 4 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x4xf32>
        %1 = arith.addf %0, %cst : f32
        affine.store %1, %arg1[%arg2, %arg3] : memref<4x4xf32>
      }
    }
    return
  }
  func.func @main_kernel_14(%arg0: memref<4x4xf32>, %arg1: memref<4x4xf32>) attributes {Kernel, main_kernel_14} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 3.40282347E+38 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 4 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x4xf32>
        %1 = arith.cmpf olt, %0, %cst : f32
        %2 = arith.select %1, %cst, %0 : f32
        %3 = arith.cmpf olt, %cst_0, %0 : f32
        %4 = arith.select %3, %cst_0, %2 : f32
        affine.store %4, %arg1[%arg2, %arg3] : memref<4x4xf32>
      }
    }
    return
  }
  func.func @main_kernel_15(%arg0: memref<1x4x2xf32>) attributes {Kernel, main_kernel_15} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg1 = 0 to 1 {
      affine.for %arg2 = 0 to 4 {
        affine.for %arg3 = 0 to 2 {
          affine.store %cst, %arg0[%arg1, %arg2, %arg3] : memref<1x4x2xf32>
        }
      }
    }
    return
  }
  func.func @main_kernel_16(%arg0: memref<1x4x4xf32>, %arg1: memref<1x4x2xf32>, %arg2: memref<1x4x2xf32>) attributes {Kernel, main_kernel_16} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg3 = 0 to 1 {
      affine.for %arg4 = 0 to 4 {
        affine.for %arg5 = 0 to 2 {
          affine.for %arg6 = 0 to 4 {
            %0 = affine.load %arg0[%arg3, %arg4, %arg6] : memref<1x4x4xf32>
            %1 = affine.load %arg1[%arg3, %arg6, %arg5] : memref<1x4x2xf32>
            %2 = affine.load %arg2[%arg3, %arg4, %arg5] : memref<1x4x2xf32>
            %3 = arith.mulf %0, %1 : f32
            %4 = arith.addf %2, %3 : f32
            affine.store %4, %arg2[%arg3, %arg4, %arg5] : memref<1x4x2xf32>
          }
        }
      }
    }
    return
  }
  func.func @main_kernel_17(%arg0: memref<4x2xf32>, %arg1: memref<4x2xf32>) attributes {Kernel, main_kernel_17} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x2xf32>
        %1 = arith.addf %0, %cst : f32
        affine.store %1, %arg1[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    return
  }
  func.func @main_kernel_18(%arg0: memref<4x2xf32>, %arg1: memref<4x2xf32>) attributes {Kernel, main_kernel_18} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x2xf32>
        %1 = math.exp %0 : f32
        affine.store %1, %arg1[%arg2, %arg3] : memref<4x2xf32>
      }
    }
    return
  }
  func.func @main_kernel_19(%arg0: memref<4xf32>) attributes {Kernel, main_kernel_19} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 0.000000e+00 : f32
    affine.for %arg1 = 0 to 4 {
      affine.store %cst, %arg0[%arg1] : memref<4xf32>
    }
    return
  }
  func.func @main_kernel_20(%arg0: memref<4x2xf32>, %arg1: memref<4xf32>) attributes {Kernel, main_kernel_20} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 2 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x2xf32>
        %1 = affine.load %arg1[%arg2] : memref<4xf32>
        %2 = arith.addf %0, %1 : f32
        affine.store %2, %arg1[%arg2] : memref<4xf32>
      }
    }
    return
  }
  func.func @main_kernel_21(%arg0: memref<4x1xf32>, %arg1: memref<4x1xf32>) attributes {Kernel, main_kernel_21} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    %cst = arith.constant 1.000000e+00 : f32
    affine.for %arg2 = 0 to 4 {
      affine.for %arg3 = 0 to 1 {
        %0 = affine.load %arg0[%arg2, %arg3] : memref<4x1xf32>
        %1 = arith.divf %cst, %0 : f32
        affine.store %1, %arg1[%arg2, %arg3] : memref<4x1xf32>
      }
    }
    return
  }
  func.func @main_kernel_22(%arg0: memref<4x2xf32>, %arg1: memref<4xf32>, %arg2: memref<4x2xf32>) attributes {Kernel, main_kernel_22} {
    cf.br ^bb1
  ^bb1:  // pred: ^bb0
    affine.for %arg3 = 0 to 4 {
      affine.for %arg4 = 0 to 2 {
        %0 = affine.load %arg0[%arg3, %arg4] : memref<4x2xf32>
        %1 = affine.load %arg1[%arg3] : memref<4xf32>
        %2 = arith.mulf %0, %1 : f32
        affine.store %2, %arg2[%arg3, %arg4] : memref<4x2xf32>
      }
    }
    return
  }
}

