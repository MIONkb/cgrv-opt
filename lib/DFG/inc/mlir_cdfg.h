...
#define GeneralOpNameFile "/home/tianyi/fdra/app-compiler/cgra-opt/lib/DFG/Documents/GeneralOpName.txt"
...
private:
    ...
    std::map<mlir::Operation*, LLVMCDFGNode*> _opNodeMap; // <Operation, node>
    ...
    std::map<LLVMCDFGNode*, llvm::StringRef> _GEPInfoMap;
    // Input/output node info map, inlcudidng variable name
    std::map<LLVMCDFGNode*, llvm::StringRef> _IOInfoMap;
public:
    ScalarEvolution *SE;
    DominatorTree *DT;
    DependenceInfo *DI;
...
///TODO:somtimes may have more than one PHINode in a level
	...
	void setName(llvm::StringRef str){_name = str;}
	llvm::StringRef name(){return _name;}
    std::string name_str(){return _name;}
    ...
    LLVMCDFGEdge* addEdge(LLVMCDFGNode *src, LLVMCDFGNode *dst);
    ...
    llvm::StringRef GEPInfo(LLVMCDFGNode* node);
    void setGEPInfo(LLVMCDFGNode* node, llvm::StringRef name);

    // Input/output node info map, inlcudidng variable name
    const std::map<LLVMCDFGNode*, llvm::StringRef>& IOInfoMap(){ return _IOInfoMap; }
    llvm::StringRef IOInfo(LLVMCDFGNode* node);
