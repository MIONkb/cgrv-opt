...
void LLVMCDFG::addNode(LLVMCDFGNode *node)
{
    ...
    errs() <<"[ADDNodes!] ";  node->instruction()->dump(); 
}
...
// get GEP node info 
llvm::StringRef LLVMCDFG::GEPInfo(LLVMCDFGNode* node)
{
...
void LLVMCDFG::setGEPInfo(LLVMCDFGNode* node, llvm::StringRef name)
{
...
llvm::StringRef LLVMCDFG::IOInfo(LLVMCDFGNode* node)
{
...
void LLVMCDFG::setIOInfo(LLVMCDFGNode* node, llvm::StringRef name)
{
...
void LLVMCDFG::initialize()
{
    ...
    for(auto BB : _loopBBs){
        errs() <<"[debug] before add I,BBs:"; BB->dump(); 
        for(auto &I : *BB){
            errs() <<"[debug] add I:"; I.dump(); 
            addNode(&I);
        }
    }
    for(auto iter = PHItoLevel.begin(); iter != PHItoLevel.end(); iter++){
        auto Ins = iter->first;
        errs() <<"[debug] add Nodes:"; Ins->dump(); 
        addNode(Ins);
    }
    // create connections
    ...
    for(auto BB : _loopBBs){
        errs() <<"\n[debug] handleBB:";BB->dump();;
        for(auto &I : *BB){
            errs() <<"\n[debug] before handle following Nodes:";I.dump();
            this->debug_printnodes();
            Instruction *ins = &I;
            ...
            if(!dyn_cast<PHINode>(ins)){
                ...
                for(Use &pred : ins->operands()){
                    ...
                    if(ConstantInt *CI = dyn_cast<ConstantInt>(pred)){
                        ...
                        // errs() <<"\n[debug] Before Const:";this->debug_printnodes();
                        inputNode = addNode("CONST", node->BB());
                        inputNode->setConstVal(CI->getSExtValue());
                        // errs() <<"\n[debug] after Const:";this->debug_printnodes();
                ...
                if(Instruction *succIns = dyn_cast<Instruction>(succ)){
                    ...
                    if(auto SI = dyn_cast<SelectInst>(succIns)){
                        ...
                        for(int i = 0; i < succIns->getNumOperands(); i++){
                            if(succNode->getInputPort(i) == node){
                                continue;
                            }
...
void LLVMCDFG::connectCtrlDepBBs(){
	...
	for(auto &elem : CtrlDepPredBBs){
		...
		for(auto &ctrlDepPredBB : elem.second){
            ...
            //     isBackEdge = true;
            // }
            _BBctrlDependentNode[currBB].insert(std::make_pair(preBRNode, cond));
...
///replace set of CTRLNodes by CTRLNOT
    for(auto &BB2preCTRLNODE : _BBctrlDependentNode){
        ...
        for(auto &elem : preCTRLs){
            ...
            errs() << "\ttem preCTRL is: "; preCTRL->instruction()->dump();
            if(cond == TRUE_COND){
                continue;
...
LLVMCDFGNode* LLVMCDFG::getInputNode(llvm::Value *ins, BasicBlock *BB)
{
...
void LLVMCDFG::handlePHINodes() 
{
	...
	for(LLVMCDFGNode* node : phiNodes){
        ...
        if((!_loopBBs.count(PHI->getParent())) && (PHItoLevel.count(PHI))){
            ...
            for(auto itr = outputs.begin(); itr != outputs.end(); itr++){
                ...
                temnode->addInputNode(selNode);
                temnode->setInputIdx(selNode, inputidx);
            ...
            selNode->addInputNode(startnode);
            selNode->setInputIdx(startnode, 2);
		...
		for (int i = 0; i < PHI->getNumIncomingValues(); ++i) {
            ...
            if(ConstantInt* CI = dyn_cast<ConstantInt>(V)){
                ...
                if(Instruction* ins = dyn_cast<Instruction>(V)){
                    ...
                    // if(dyn_cast<PHINode>(ins)){
                    if(dyn_cast<PHINode>(ins) && phiParent != NULL){
    ...
    for(auto &elem : phi2phiParentMap){
        ...
        errs()<<"[debuf] in phi2phiParentMap:\n";
        errs()<<"[debuf] phinode:"; phinode->instruction()->dump();
        errs()<<"[debuf] phiparent:"; phiparent->instruction()->dump();
...
std::map<int, int> LLVMCDFG::getStructElemOffsetMap(StructType *ST)
{
    ...
    int offset = 0;
    int idx = 0;
    for(llvm::Type *type : ST->elements()){
...
void LLVMCDFG::handleGEPNodes()
{
    ...
    for(auto node : GEPNodes){
        errs() <<"[debug] GEP:"; node->instruction()->dump(); 
        auto ins = node->instruction();
        ...
        std::vector<std::pair<llvm::Value*, int>> varOperandSizes; // <non-constant-operand, element-size>
        int offset = 0;
...
LLVMCDFGNode* LLVMCDFG::addNodeTree(llvm::Value* opnode){
...
std::set<std::pair<LLVMCDFGNode*, int>> LLVMCDFG::addOutputTree(llvm::Value* user){
    ...
    for (int i = 0; i < temins->getNumOperands(); i++)
    {
        ...
        if (dyn_cast<Constant>(preopnode))
        {
            if (temnode == NULL)
            {
                ...
                if(edge(prenode, temnode) != NULL && temnode->getInputPort(i) == prenode){
                    continue;
                }
                temnode->addInputNode(prenode, i);
            ...
            if (temnode != NULL)
            {
                ...
                {   
                    if(!hasSELECT){ 
                        ...
                        if(edge(prenode, temnode) != NULL && temnode->getInputPort(i) == prenode){//this edge is here
                            continue;
                        }
                        temnode->addInputNode(prenode, i);
                        prenode->addOutputNode(temnode);
...
//get every nests' idx stride & bound
    for(int level = 0; level < total_level; level++){
        ...
        llvm::Value* indexbegin, *indexend;
...
*/
        ...
        for(auto BI = temNestBBs.begin(); BI != temNestBBs.end(); BI++){
            ...
            if (detected.find(temBB) == detected.end())
            {
                ...
                {
                    Instruction *ins = &*II;
                    errs()<<"[debug] Instruction:";ins->dump();
                    if (auto Phi = dyn_cast<PHINode>(ins))
                    {
                        if(Phi != idxPhi){
                            continue;
                            ...
                            if (temNestBBs.find(PhiNext) != temNestBBs.end())
                            {
                                ...
                                if (isloopsAffine())
                                {
                                    ...
                                    for (auto phiusers : Phi->users())
                                    {
                                        ...
                                        {
                                            errs() << "[debug]PHI node users: "; I->dump();
                                            if (auto cmp = dyn_cast<ICmpInst>(I))
                                            {
                                                if(cmp != exitcmp)
                                                    continue;
                                                errs() << "[debug] cmp node is: "; cmp->dump();
                                                flag = false;
                                        ...
                                        for (auto opusers : Phi->getIncomingValueForBlock(PhiNext)->users())
                                        {
                                            if (auto I = dyn_cast<Instruction>(opusers))
                                            {
                                                if (auto cmp = dyn_cast<ICmpInst>(I))
                                                {
                                                    ...
                                                    if(auto truenext = branch->getSuccessor(0)){
                                                        errs()<<Phi->getName().str()<<" is in "<<Phi->getParent()->getName().str()<<"\n";
                                                        errs()<<"[debug] true next is ";truenext->dump();
                                                        if(truenext == Phi->getParent()){///cmp is true, but loop repeat
                                                            errs()<<"[debug] Phi->getParent() is ";Phi->getParent()->dump();
                                                            bounds[level].second = BinaryConstOp(I).second + stride;
                                                            errs() << "\t"<< cmp->getName().str()<<" is true, but loop repeat; bounds[level].second = "<<bounds[level].second << "\n";
                                                        }
                                                    }
...
std::vector<PHINode*> LLVMCDFG::arrayStride(llvm::Value* opnode, std::map<int, std::pair<double,std::pair<double,double>>>* factortable){
...
void LLVMCDFG::affineAnalyze(){
        ...
        for(User *succ : ins->users()){               
            if(Instruction *succIns = dyn_cast<Instruction>(succ)){
                if(auto succNode = node(succIns)){
                    ...
                    // succNode->addInputNode(node, idx, isBackEdge);
                    // addEdge(node, succNode, type);
                    if(edge(phinode, succNode) != NULL && succNode->getInputIdx(phinode) == idx){
                        continue;//if this edge has been added----continue
                    }
    ...
    for(auto &elem : _nodes){
        ...
        int dimension = GEP->getNumIndices();
        errs()<<"[debug] dimension: "<< dimension << ", GEP:";GEP->dump();
        if(!_loopBBs.count(GEP->getParent())){//GEP or its direct operand is outer
            ...
            if(LLVMCDFG::node(GEP) == NULL)
                newnode = addNodeTree(dyn_cast<llvm::Value>(GEP));
            else
        ...
        for(int i = dimension; i <= dimension; i++){ //////////////// To FIx!!
        ...
        for(int temdi = dimension; temdi > dimension - 1; temdi--){ //// TO fix
            /*
            ...
            */
            ...
            llvm::Value *arrayindex = GEP->getOperand(temdi);
        ...
        if(isGEPaffine){
            ...
            for(int i = 2; i <= dimension; i++){
                currTypesize = currTypesize*(1/arrayscale[i]);
                // currTypesize/arrayscale[i];
                LSbounds[i-2][0] = 0;
...
void LLVMCDFG::handleAffineLSNodes(){
    ...
    for(auto &elem : nodes){
        ...
        if(node->customInstruction() == "LOOPEXIT"){
            delNode(node);
        }
    }
...
void LLVMCDFG::handleSelectNodes(){
    std::map<int, LLVMCDFGNode *> nodes = _nodes;
    std::vector<LLVMCDFGNode*> rmnodes;
    for (auto &elem : nodes)//Condition(all LS affine)'s SELECT delete has been dealed integrated below
    {
        auto node = elem.second;
        if (node->customInstruction() == "SELECT")
        {
            ...
            auto teminputs = node->inputNodes();
            std::map<int, LLVMCDFGNode*> IdxToInput;
            for (auto pre : teminputs){
                IdxToInput[node->getInputIdx(pre)] = pre;
            }
            errs() <<"IdxToInput size is: "<< IdxToInput.size() << "\n";
            assert(IdxToInput.size() == 3);
            for (int i = 0; i < 3; i++)
            {
                auto pre = IdxToInput[i];
                ...
                /// TODO:need check is initial condition complete
                else if (pre->hasConst() && i == 0 && IdxToInput[2]->customInstruction() == "LOOPSTART")///if exist CONST && controled by LOOPSTART;
                    ...
                    else
                    {
                        if (preins->getOpcode() == Instruction::Add)
                        {
                            ...
                            for(auto eid: temoutputedges){
                                ...
                                if(selectout->instruction()->getOpcode() != Instruction::Add){//only consider serial addition
                            ...
                            if(cycleout != pre && legalexchange){//exchange add oprands
    ...
    for (auto &elem : _nodes)//check if SELECT nodes left forming conditional select
    {
        auto node = elem.second;
        if (node->customInstruction() == "SELECT"){
            if(node->hasInitial()){//ACC's prerequisites
                ...
                assert(node->inputNodes().size() == 1 && SelParent1 != NULL);
                ...
                for(auto selectout : temoutputs){
                    ...
                    if(selectout->instruction()->getOpcode() != Instruction::Add){//only consider serial addition
                        ...
                        while(!findCACC && !todetect.empty()){
                            ...
                            detected.insert(temnode);
                            todetect.pop_back();
                            if(temnode == SelParent1){//when find SelParent1 in cycle, its inputnode in path will be preADD
                                findCACC = true;
                ...
                { // exchange add oprands
                    ...
                    preADD->addInputNode(node);
                    addEdge(node, preADD, EDGE_TYPE_DATA);
                    preADD->setInputIdx(node, preidx);
                ...
                node->addInputNode(condition, 2);
                addEdge(condition, node, EdgeType::EDGE_TYPE_CTRL);
                bool CACCfirst = true;
                for(auto result : SelParent1->outputNodes()){//connect result out to CACC
                    if(result != node){
                ...
                if(ADDNode->outputNodes().size() == 0){///ADD result only output to SelParent; then delnode
                    delNode(ADDNode);
                }
                //rename CACC
...
void LLVMCDFG::generateCDFG()
{
    ...
    createCtrlOrTree();
    printDOT("_0_"+_name + "_after_createCtrlOrTree.dot");

    outs() << ">>>>>> Transfer GEP node to MUL/ADD/Const tree\n";
    handleGEPNodes();
    printDOT("_1_"+_name + "_after_handleGEPNodes.dot");
    

    errs()<<"[debug] Before handlePHI:";
    this->debug_printnodes();
    outs() << ">>>>>> Transfer PHINode to SELECT nodes\n";
    handlePHINodes(); 
    printDOT("_2_"+_name + "_after_handlePHINodes.dot");

    outs() << ">>>>>> affine Access Node\n";
    handleAffineLSNodes(); 
    printDOT("_3_"+_name + "_after_handleAffineLSNodes.dot");

    outs() << ">>>>>> Handle select Node\n";
    handleSelectNodes(); 
    printDOT("_4_"+_name + "_after_handleSelectNodes.dot");

    outs() << ">>>>>> Remove redundant nodes\n";
    removeRedundantNodes();
    printDOT("_5_"+_name + "_after_removeRedundantNodes.dot");
...
/****
 * 20220310
 * ADDED By LJH to debug:
 * print all nodes
 * 
*/
void LLVMCDFG::debug_printnodes(){
    errs() <<"[debug] current nodes:\n";
	for(auto &elem : _nodes){
        auto node = elem.second;
        errs() <<"[debug] nodes:";
        if(node->hasConst() || node->instruction() == NULL){
            errs()<< node->customInstruction() << " " << node->constVal() << "\n";
        }
        else{
            node->instruction()->dump();    
        }
    }
}
 ...
 * 
*/
void LLVMCDFG::debug_printBBs(){
    errs() <<"[debug] current BBs:\n";
    for(auto BB : _loopBBs){
        errs()<< "[debug] BB: ";BB->dump();
    }
}