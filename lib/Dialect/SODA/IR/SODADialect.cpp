...
void SODAFuncOp::build(OpBuilder &builder, OperationState &result,
  ...
  Region *body = result.addRegion();
  // std::cout << "[debug] after addRegion body:\n";body->dump();
  ...
  for (Type argTy : type.getInputs()){
    ...
    std::cout << "[debug] after type.getInputs() entryBLock:\n";entryBlock->dump();
  }

  for (Type argTy : workgroupAttributions){
    entryBlock->addArgument(argTy, result.location);
    std::cout << "[debug] after workgroupAttributions entryBLock:\n";entryBlock->dump();
  }
  for (Type argTy : privateAttributions)
  {
    entryBlock->addArgument(argTy, result.location);
    std::cout << "[debug] after privateAttributions entryBLock:\n";entryBlock->dump();
  }
